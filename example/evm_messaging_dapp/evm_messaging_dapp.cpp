...
EvmMessagingDapp::EvmMessagingDapp(
    const std::vector<rapidjson::Document>                                             &configs,
    const std::vector<std::string>                                                     &contractAddresses,
    const std::vector<std::vector<sgns::evmwatcher::EvmMessagingWatcher::TopicFilter>> &topicFilters )
{
    if ( configs.size() != contractAddresses.size() || configs.size() != topicFilters.size() )
    {
        throw std::runtime_error(
            "The number of configurations, contract addresses, and topic filter sets must match" );
    }
    ...
    {
        auto evmWatcher = std::make_shared<sgns::evmwatcher::EvmMessagingWatcher>( configs[i],
...
int main()
{
    ...
    std::vector<std::string> contractAddresses = { "0x1234567890abcdef1234567890abcdef12345678" };
