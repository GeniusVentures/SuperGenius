...
#include <filesystem> // For finding expert files
#include <regex>      // For filename pattern matching
...
// NEW CLASS: MultiExpertHandler to manage multiple expert models
...
public:
    MultiExpertHandler() : initialized( false ), expertStrategy( 0 ), debugMode( false )
    {
        // Default config
    ...
    void setStrategy( int strategy )
    {
        if ( strategy >= 0 && strategy <= 1 )
        {
            ...
            std::cout << "Expert strategy set to: " << ( expertStrategy == 0 ? "Round Robin" : "Average All" )
                      << std::endl;
        }
    }
    ...
    bool loadExpertModel( int index )
    {
        ...
        try
        {
            ...
            {
                experts[index].interpreter.reset(
                    MNN::Interpreter::createFromFile( experts[index].modelPath.c_str() ) );
                if ( !experts[index].interpreter )
                {
    ...
    std::vector<float> runSingleExpert( int expertIndex, const std::vector<float> &embedding )
    {
        ...
        {
            // Get input tensor
            auto expertInput = experts[expertIndex].interpreter->getSessionInput( experts[expertIndex].session,
                                                                                  nullptr );
            if ( !expertInput )
            {
                throw std::runtime_error( "Failed to get input tensor" );
            ...
            if ( debugMode )
            {
                saveTensorToFile( expertOutputTensor, "expert" + std::to_string( expertIndex ) + "_output.bin" );
            }
