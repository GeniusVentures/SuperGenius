...
public:
                      ...
                      const sgns::CID                             &root_cid )
    {
        ...
        requests_.push_back( std::shared_ptr<sgns::ipfs_lite::ipfs::graphsync::Subscription>(
            new sgns::ipfs_lite::ipfs::graphsync::Subscription(
                std::move( graphsync_->makeRequest( peer,
                                                    std::move( addresses ),
                                                    root_cid,
                                                    {},
                                                    extensions,
...
private:
    ...
    sgns::ipfs_lite::ipfs::graphsync::Graphsync::RequestProgressCallback requestProgressCallback()
    {
            ...
            []( const std::vector<sgns::ipfs_lite::ipfs::graphsync::Extension> &extensions ) -> std::string
        {
            ...
            {
                s += fmt::format( "({}: 0x{}) ", item.name, sgns::common::Buffer( item.data ).toHex() );
            }
            return s;
        };
...
void RunIpfsServer()
{
    ...
    auto listen_to = libp2p::multi::Multiaddress::create( "/ip4/127.0.0.1/tcp/40000/ipfs/" + server.getId().toBase58() )
                         .value();
...
void RunIpfsClient( std::string remote )
{
    ...
    auto listen_to = libp2p::multi::Multiaddress::create( "/ip4/127.0.0.1/tcp/40000/ipfs/" + client.getId().toBase58() )
                         .value();
    ...
    logger->info( "total requests sent {}, responses received {}", Node::requests_sent, Node::responses_received );
}
...
void testManyNodesExchange( size_t N, size_t n_data )
{
    ...
    for ( size_t i = 0; i < N; ++i )
    {
        ...
        p.listen_to = libp2p::multi::Multiaddress::create( fmt::format( "/ip4/127.0.0.1/tcp/{}", 40000 + i ) ).value();
    ...
    for ( size_t i = 0; i < N; ++i )
    {
        ...
        auto cb =
            [ds = p.data_service, &expected, &unexpected_responses, &total_responses, &io]( sgns::CID            cid,
                                                                                            sgns::common::Buffer data )
        {
...
namespace
{
    ...
    boost::optional<Options> parseCommandLine( int argc, char **argv )
    {
        ...
        try
        {
            ...
            desc.add_options()( "help,h", "print usage message" )( "mode,m",
                                                                   po::value( &o.mode ),
                                                                   "application mode (client/server)" )(
                "remote,r",
                po::value( &remote ),
