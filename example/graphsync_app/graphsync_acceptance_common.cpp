...
createNodeObjects( std::shared_ptr<boost::asio::io_context> io )
{
    ...
    std::pair<std::shared_ptr<sgns::ipfs_lite::ipfs::graphsync::Graphsync>, std::shared_ptr<libp2p::Host>> objects;
...
void TestDataService::insertNode( TestDataService::Storage &dst, const std::string &data_str )
{
...
outcome::result<size_t> TestDataService::select(
    ...
    std::function<bool( const sgns::CID &cid, const sgns::common::Buffer &data )> handler ) const
{
