...
struct Cmp
{
    bool operator()( const std::shared_ptr<Session> &lhs, const std::shared_ptr<Session> &rhs ) const;
};
...
public:
    ...
    bool read()
    {
        ...
        stream_->readSome( gsl::span( incoming_->data(), static_cast<ssize_t>( incoming_->size() ) ),
                           incoming_->size(),
                           [self = shared_from_this()]( libp2p::outcome::result<size_t> result )
                           {
                               ...
                               std::cout << self->stream_->remotePeerId().value().toBase58() << " > "
                                         << std::string(
                                                ...
                                                self->incoming_->begin() + static_cast<ssize_t>( result.value() ) );
                               std::cout.flush();
                               self->read();
                           } );
        return true;
    ...
    bool write( const std::shared_ptr<std::vector<uint8_t>> &buffer )
    {
        ...
        stream_->write( gsl::span( buffer->data(), static_cast<ssize_t>( buffer->size() ) ),
                        buffer->size(),
                        [self = shared_from_this(), buffer]( libp2p::outcome::result<size_t> result )
                        {
                            if ( !result )
                            {
                                self->close();
                                std::cout << self->stream_->remotePeerId().value().toBase58() << " - closed at writting"
                                          << std::endl;
                                return;
                            }
                            std::cout << self->stream_->remotePeerId().value().toBase58() << " < "
                                      << std::string( buffer->begin(),
                                                      buffer->begin() + static_cast<ssize_t>( result.value() ) );
                            std::cout.flush();
                        } );
        return true;
    }
    ...
    {
        return stream_->remotePeerId().value() < other.stream_->remotePeerId().value();
    }
...
bool Cmp::operator()( const std::shared_ptr<Session> &lhs, const std::shared_ptr<Session> &rhs ) const
{
    return *lhs < *rhs;
}
...
void handleIncomingStream( libp2p::protocol::BaseProtocol::StreamResult stream_res )
{
    if ( !stream_res )
    {
        std::cerr << " ! incoming connection failed: " << stream_res.error().message() << std::endl;
        return;
    }
    ...
    std::cout << stream->remotePeerId().value().toBase58() << " + incoming stream from "
              << stream->remoteMultiaddr().value().getStringAddress() << std::endl;
...
void handleOutgoingStream( libp2p::protocol::BaseProtocol::StreamResult stream_res )
{
    if ( !stream_res )
    {
        std::cerr << " ! outgoing connection failed: " << stream_res.error().message() << std::endl;
        return;
    }
    auto &stream = stream_res.value();
    ...
    std::cout << stream->remotePeerId().value().toBase58() << " + outgoing stream to "
              << stream->remoteMultiaddr().value().getStringAddress() << std::endl;
    ...
    try
    {
        ...
        std::function<void()> find_providers = [&]
        {
            [[maybe_unused]] auto res1 = kademlia->findProviders(
                ...
                {
                    scheduler
                        .schedule( libp2p::protocol::scheduler::toTicks( kademlia_config.randomWalk.interval ),
                                   find_providers )
                        .detach();
        ...
        {
            [[maybe_unused]] auto res = kademlia->provide( content_id.value(), !kademlia_config.passiveMode );

            scheduler.schedule( libp2p::protocol::scheduler::toTicks( kademlia_config.randomWalk.interval ), provide )
                .detach();
        };
