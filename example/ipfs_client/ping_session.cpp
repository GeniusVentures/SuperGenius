...
void PingSession::Init()
{
    subsOnNewConnection_ = host_->getBus().getChannel<libp2p::event::network::OnNewConnectionChannel>().subscribe(
        [ctx = shared_from_this()]( auto &&conn ) mutable { return ctx->OnNewConnection( conn, ctx->ping_ ); } );

    host_->setProtocolHandler( ping_->getProtocolId(),
...
void PingSession::OnNewConnection( const std::weak_ptr<libp2p::connection::CapableConnection> &conn,
