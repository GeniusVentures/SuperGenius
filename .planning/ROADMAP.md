...
**Plans:** 4/5 plans complete — 12-05 pending re-run: prior attempt recorded a deterministic `bridge_race_single_burn_test` failure (mint output destination mismatch); root cause fixed in evmrelay `4787e582` (bridge destination byte order), full-suite verification still owed
