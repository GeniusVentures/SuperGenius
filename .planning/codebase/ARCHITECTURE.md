...
- **Global state:** `CComponentFactory` is a `CSingleton` (Meyers singleton) serving as the global DI registry. `DevConfig_st DEV_CONFIG` is an extern global in `src/account/GeniusNode.hpp`. `CSingleton<T>` template provides global-instance accessor in `src/singleton/Singleton.hpp`.
