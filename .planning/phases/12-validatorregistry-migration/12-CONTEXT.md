...
- **D-01:** The migration touches exactly the call sites in `ValidatorRegistry.cpp` that verify a raw signature against a payload — `VerifyUpdate`'s genesis path (`cpp:1387-1406`, currently loops `update.signatures()` checking `validator_id() == genesis_authority_` + `GeniusAccount::VerifySignature`) and `StoreGenesisRegistry`'s signing counterpart (`cpp:621-625`). These become calls to `multisig::VerifyPayloadSignature`. No other code path changes.
