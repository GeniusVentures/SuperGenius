...
- **D-04:** Both `InitializeRpcEndpoints()` and `BridgeRelayer::Start()` fire as a single async function from `CREATING` state. No new state machine state. Non-blocking — node proceeds through normal transitions. Ordering guaranteed: Start() only fires after endpoints are ready.
