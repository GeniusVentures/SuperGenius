...
- Link-dependency check (corrected per verifier): `src/multisig/CMakeLists.txt` itself links only `sgns_genius_account` (no direct crdt/pubsub/genius_node dependency) — the plan's actual acceptance criterion. Note the test *executables'* `link.txt` do transitively pull in `crdt_globaldb`/`ipfs-pubsub` because `sgns_genius_account` (a pre-existing, unrelated target predating Phase 8) links those `PUBLIC`. MSIG-03's functional independence still holds — the multisig code itself never instantiates CRDT/node/network — but the earlier claim of zero transitive link presence in the test binaries was inaccurate and is corrected here.
