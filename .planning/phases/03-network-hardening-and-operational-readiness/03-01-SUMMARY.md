...
provides:
  ...
  - "Configurable timestamp tolerance via DevConfig_st → TransactionManager (TS-01)"
...
tech-stack:
  ...
  patterns:
    ...
    - "TS-01: DevConfig_st aggregate-init default → GeniusNode::New → SetTimeFrameToleranceMs → CheckTransactionTimestamp config chain"
...
key-files:
  ...
  modified:
    ...
    - "src/account/GeniusNode.hpp — timestamp_tolerance_ms field in DevConfig_st (1 line)"
...
key-decisions:
  ...
  - "TS-01 adds timestamp_tolerance_ms to DevConfig_st with default 300000ms, wired during GeniusNode::New() after Start() — no new CLI option needed"
...
patterns-established:
  ...
  - "Config flow: DevConfig_st field → GeniusNode::New → TransactionManager setter → member variable read by validation function"
...
**64KB pre-publish size enforcement, configurable timestamp tolerance via DevConfig_st, and 7 atomic metrics counters with lifecycle logging and destructor flush**
...
- **TS-01:** Timestamp tolerance made configurable via `DevConfig_st.timestamp_tolerance_ms` (default 300000ms). Wired through `GeniusNode::New()` → `SetTimeFrameToleranceMs()` → `CheckTransactionTimestamp` reads configurable value. No hardcoded literal in the validation path.
...
- `src/account/GeniusNode.hpp` — `DevConfig_st` gains `uint64_t timestamp_tolerance_ms = 300000` field with Doxygen comment
...
- Added `timestamp_tolerance_ms` to existing `DevConfig_st` struct rather than creating new config mechanism — follows D-06 (existing codebase patterns) and avoids CLI option parsing complexity
