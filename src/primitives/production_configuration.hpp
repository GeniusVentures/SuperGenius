...
namespace sgns::primitives
{
    ...
    {
        size_t slot_duration_u64 = std::chrono::duration_cast<std::chrono::milliseconds>( config.slot_duration )
                                       .count();
        return s << slot_duration_u64 << config.epoch_length << config.leadership_rate << config.genesis_authorities
                 << config.randomness;
    }
    ...
    Stream &operator>>( Stream &s, ProductionConfiguration &config )
    {
        ...
        s >> slot_duration_u64 >> config.epoch_length >> config.leadership_rate >> config.genesis_authorities >>
            config.randomness;
