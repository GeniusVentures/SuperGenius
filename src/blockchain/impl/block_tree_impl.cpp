...
namespace sgns::blockchain
{
    ...
    std::shared_ptr<BlockTreeImpl::TreeNode> BlockTreeImpl::TreeNode::getByHash( const primitives::BlockHash &hash )
    {
        // standard BFS
    ...
    bool BlockTreeImpl::TreeNode::operator==( const TreeNode &other ) const
    {
        ...
        auto        parents_equal = ( parent.expired() && other_parent.expired() ) ||
                             ( !parent.expired() && !other_parent.expired() && parent.lock() == other_parent.lock() );

        return parents_equal && block_hash == other.block_hash && depth == other.depth;
    }
    ...
    {
        std::function<void( std::shared_ptr<TreeNode> )> handle = [&]( std::shared_ptr<TreeNode> node )
        {
    ...
    BlockTreeImpl::TreeMeta::TreeMeta( std::unordered_set<primitives::BlockHash> leaves,
                                       TreeNode                                 &deepest_leaf,
                                       TreeNode                                 &last_finalized ) :
        leaves{ std::move( leaves ) }, deepest_leaf{ deepest_leaf }, last_finalized{ last_finalized }
    {
    }
        ...
        std::shared_ptr<crypto::Hasher>             hasher )
    {
        ...
        auto tree = std::make_shared<TreeNode>( hash_res, header.number, nullptr, true );
    ...
    BlockTreeImpl::BlockTreeImpl( std::shared_ptr<BlockHeaderRepository>      header_repo,
                                  std::shared_ptr<BlockStorage>               storage,
    ...
    outcome::result<void> BlockTreeImpl::addBlockHeader( const primitives::BlockHeader &header )
    {
        ...
        auto new_node = std::make_shared<TreeNode>( block_hash, header.number, parent );
    ...
    outcome::result<void> BlockTreeImpl::addBlock( const primitives::Block &block )
    {
        ...
        // update local meta with the new block
        auto new_node = std::make_shared<TreeNode>( block_hash, block.header.number, parent );
        parent->children.push_back( new_node );
    ...
    outcome::result<void> BlockTreeImpl::addBlockBody( primitives::BlockNumber      block_number,
    ...
    outcome::result<void> BlockTreeImpl::finalize( const primitives::BlockHash     &block,
                                                   const primitives::Justification &justification )
    {
        ...
        BOOST_OUTCOME_TRY( storage_->putJustification( justification, block, node->depth ) );

        // update our local meta
        node->finalized = true;

        BOOST_OUTCOME_TRY( prune( node ) );
        ...
        BOOST_OUTCOME_TRY( storage_->setLastFinalizedBlockHash( node->block_hash ) );

        log_->info( "Finalized block number {} with hash {}", node->depth, block.toHex() );
        return outcome::success();
    }

    outcome::result<primitives::BlockHeader> BlockTreeImpl::getBlockHeader( const primitives::BlockId &block ) const
    {
        return storage_->getBlockHeader( block );
    }

    outcome::result<primitives::BlockBody> BlockTreeImpl::getBlockBody( const primitives::BlockId &block ) const
    {
    ...
    BlockTreeImpl::BlockHashVecRes BlockTreeImpl::getChainByBlock( const primitives::BlockHash &block )
    {
                                                                   ...
                                                                   uint64_t                     maximum )
    {
        ...
        if ( !block_number_res )
        {
            log_->error( "cannot retrieve block with hash {}: {}", block, block_number_res.error().message() );
            return BlockTreeError::NO_SUCH_BLOCK;
        }
    ...
    BlockTreeImpl::BlockHashVecRes BlockTreeImpl::getChainByBlocks( const primitives::BlockHash &top_block,
    ...
    bool BlockTreeImpl::hasDirectChain( const primitives::BlockHash &ancestor, const primitives::BlockHash &descendant )
    {
        ...
        const boost::optional<primitives::BlockNumber> &max_number ) const
    {
        ...
        BOOST_OUTCOME_TRY( auto canon_hash, header_repo_->getHashByNumber( target_header.number ) );
        ...
        if ( canon_hash == target_hash )
        {
            if ( max_number.has_value() )
            {
                ...
                if ( header )
                {
                    BOOST_OUTCOME_TRY( auto hash, header_repo_->getHashByNumber( header.value().number ) );
        ...
        {
            BOOST_OUTCOME_TRY( auto last_finalized, header_repo_->getNumberByHash( getLastFinalized().block_hash ) );
        ...
        log_->warn( "Block {:?} exists in chain but not found when following all \
    ...
    BlockTreeImpl::BlockHashVecRes BlockTreeImpl::getChildren( const primitives::BlockHash &block )
    {
        auto node = tree_->getByHash( block );
        if ( !node )
        {
    ...
    std::vector<primitives::BlockHash> BlockTreeImpl::getLeavesSorted() const
    {
        ...
        std::sort( leaf_depths.begin(),
                   leaf_depths.end(),
                   []( auto const &p1, auto const &p2 ) { return p1.block_number > p2.block_number; } );
    ...
    void BlockTreeImpl::collectDescendants(
        ...
        std::vector<std::pair<primitives::BlockHash, primitives::BlockNumber>> &container )
    {
