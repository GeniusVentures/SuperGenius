...
namespace sgns::blockchain
{
    ...
    KeyValueBlockStorage::KeyValueBlockStorage( std::shared_ptr<crdt::GlobalDB>                db,
        ...
        db_{ std::move( db ) }, hasher_{ std::move( hasher ) }, logger_{ base::createLogger( "Block Storage:" ) },
    ...
    {
        auto block_storage =
            std::make_shared<KeyValueBlockStorage>( KeyValueBlockStorage( db, std::move( hasher ), header_repo ) );
    ...
    outcome::result<void> KeyValueBlockStorage::setLastFinalizedBlockHash( const primitives::BlockHash &hash )
    {
        ...
        BOOST_OUTCOME_TRYV2( auto &&,
                             db_->Put( { std::string( ( storage::kLastFinalizedBlockHashLookupKey ).toString() ) },
                                       Buffer{ serialized_proto } ) );
