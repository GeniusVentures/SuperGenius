...
namespace sgns
{
    ...
    std::shared_ptr<Blockchain> Blockchain::New(
        std::shared_ptr<crdt::GlobalDB> global_db, 
        std::shared_ptr<GeniusAccount> account)
    {
    ...
    void Blockchain::Start(GenesisCallback callback)
    {
        ...
        if ( get_genesis_result.has_error() )
        {
            ...
            else
            {
                ...
                // Use weak_ptr to safely capture this - now safe since always shared_ptr
                std::weak_ptr<Blockchain> weak_self = shared_from_this();
                account_->RequestGenesis([weak_self](const std::string& serialized_genesis) {
                    if (auto self = weak_self.lock()) {
                        self->OnGenesisBlockReceived(serialized_genesis);
                    }
                    // If self is nullptr, the Blockchain was destroyed - safely ignore
                });
            }
        }
        else
