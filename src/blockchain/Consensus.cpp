...
namespace sgns
{
    ...
    void ConsensusManager::ProcessCertificates()
    {
        ...
        for ( auto &state : to_process )
        {
            {
                std::lock_guard lock( proposals_mutex_ );
                if ( restored_safety_slots_.count( state.slot_key ) != 0 ) continue;
            }
            ...
            if ( subject_hash.has_value() && CheckCertificateForSubject( subject_hash.value() ) )
            {
                ...
                if ( authoritative ) (void) FinalizeSlot( authoritative.value(), DeliverySource::Recovery );
                continue;
            }
    ...
    bool ConsensusManager::RegisterCertificateFilter()
    {
        ...
        const bool delta_filter_registered = db_->RegisterDeltaFilter(
        ...
        return delta_filter_registered && filter_registered && callback_registered;
    }
    ...
    void ConsensusManager::HandleVote( const Vote &vote )
    {
        ...
        bool has_quorum = false;
        {
            ...
            if ( restored_safety_slots_.count( proposal_state.slot_key ) != 0 )
            {
                ConsensusManagerLogger()->critical(
                    "{}: vote aggregation blocked by slot safety violation slot={} proposal_id={}",
                    __func__, proposal_state.slot_key, vote.proposal_id() );
                return;
            }
