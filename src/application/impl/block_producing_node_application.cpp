...
{
    BlockProducingNodeApplication::BlockProducingNodeApplication( const AppConfigPtr &app_config ) :
        ...
        app_state_manager_->atLaunch(
            [this]
            {
                ...
                io_context_->post(
                    [this]
                    {
                        const auto &current_peer_info = injector_.template create<network::OwnPeerInfo>();
                        ...
                        for ( const /*auto*/ libp2p::multi::Multiaddress &ma : current_peer_info.addresses )
                        {
                            ...
                            if ( !listen )
                            {
                                logger_->error( "Cannot listen address {}. Error: {}", ma.getStringAddress(),
