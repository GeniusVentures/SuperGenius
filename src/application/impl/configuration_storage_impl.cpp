  ...
  outcome::result<void> ConfigurationStorageImpl::loadFields(
    ...
    if (fork_blocks_opt.has_value()
      ...
      for (auto &[_, fork_block] : fork_blocks_opt.value()) {
        // TODO: Ensure if implementation is correct, and remove return
    ...
    auto verification_engine_opt = tree.get_child_optional("verificationEngine");
    if (verification_engine_opt.has_value()) {
      auto verification_engine = verification_engine_opt.value().get<std::string>("");
      if (verification_engine != "null") {
        verification_engine_.emplace(std::move(verification_engine));
      }
    }
