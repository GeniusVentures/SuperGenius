...
namespace sgns::network {
  RouterLibp2p::RouterLibp2p(
      ...
      : host_{host},
        production_observer_{std::move(production_observer)},
        finality_observer_{std::move(finality_observer)},
        sync_observer_{std::move(sync_observer)},
        extrinsic_observer_{std::move(extrinsic_observer)},
        gossiper_{std::move(gossiper)},
        log_{base::createLogger("RouterLibp2p")} {
    BOOST_ASSERT_MSG(production_observer_ != nullptr, "production observer is nullptr");
    BOOST_ASSERT_MSG(finality_observer_ != nullptr,
                     "finality observer is nullptr");
  ...
  bool RouterLibp2p::processGossipMessage(const GossipMessage &msg) const {
    ...
    switch (msg.type) {
      case MsgType::BLOCK_ANNOUNCE: {
        ...
        production_observer_->onBlockAnnounce(msg_res.value());
        return true;
      }
      case MsgType::VERIFICATION: {
        auto vote_msg_res =
            scale::decode<verification::finality::VoteMessage>(msg.data);
        if (vote_msg_res) {
          finality_observer_->onVoteMessage(vote_msg_res.value());
          return true;
        }

        auto fin_msg_res = scale::decode<verification::finality::Fin>(msg.data);
        if (fin_msg_res) {
          finality_observer_->onFinalize(fin_msg_res.value());
          return true;
        }

        log_->error("error while decoding a verification message");
        return false;
      }
