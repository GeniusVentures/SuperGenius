...
OUTCOME_CPP_DEFINE_CATEGORY_3( sgns::trustedpeer, TrustStateStore::Error, e )
{
    ...
    switch ( e )
    {
        case Error::NOT_FOUND: return "confirmed trust state not found";
        case Error::ALREADY_INITIALIZED: return "trust state is already initialized";
        case Error::NETWORK_MISMATCH: return "persisted trust state belongs to another network";
        case Error::CORRUPT_GENESIS: return "persisted genesis record is corrupt";
        case Error::CORRUPT_FINGERPRINT: return "persisted genesis fingerprint is corrupt";
        case Error::INVALID_GENESIS_PROOF: return "persisted genesis proof is invalid";
        case Error::MISSING_POLICY_RECORD: return "policy head references a missing record";
        case Error::MISSING_BURN_RECORD: return "burn head references a missing record";
        case Error::CORRUPT_POLICY_RECORD: return "persisted policy record is corrupt";
        case Error::CORRUPT_BURN_RECORD: return "persisted burn record is corrupt";
        case Error::INVALID_POLICY_PROOF: return "persisted policy proof is invalid";
        case Error::INVALID_BURN_PROOF: return "persisted burn proof is invalid";
        case Error::VERSION_DECREASE: return "candidate version does not advance the durable version";
        case Error::VERSION_SKIP: return "candidate version skips the next durable version";
        case Error::WRONG_PREDECESSOR: return "candidate does not descend from the durable head";
        case Error::WRONG_AUTHORIZER: return "candidate is not authorized by the durable policy head";
        case Error::STALE_HEAD: return "candidate lost the durable-head transition race";
        case Error::COMMIT_FAILED: return "synchronous trust-state batch commit failed";
    }
...
namespace sgns::trustedpeer
{
    namespace
    {
        ...
        std::string GenesisKey( uint16_t network_id ) { return Prefix( network_id ) + "genesis"; }
        std::string PolicyHeadKey( uint16_t network_id ) { return Prefix( network_id ) + "policy/head"; }
        std::string BurnHeadKey( uint16_t network_id ) { return Prefix( network_id ) + "burn/head"; }
        ...
        std::optional<std::pair<uint64_t, std::string>> DecodeHead( const base::Buffer &bytes )
        {
            ...
            if ( !version || *version == 0 || !hash || !reader.Exhausted() ) return std::nullopt;
            std::string hash_string( hash->begin(), hash->end() );
            if ( !IsHash( hash_string ) ) return std::nullopt;
        ...
        std::optional<multisig::CollectedSignatures> DecodeProof( CanonicalTrustCodec::Reader &reader )
        {
            ...
            if ( !count || *count == 0 || *count > CanonicalTrustCodec::MAX_TRUSTED_PEERS ) return std::nullopt;
            ...
            for ( uint32_t index = 0; index < *count; ++index )
            {
                ...
                if ( !signer || signer->size() != CanonicalTrustCodec::PUBLIC_KEY_BYTES * 2 || !signature ||
                     signature->empty() ) return std::nullopt;
                                                   ...
                                                   const std::vector<uint8_t> &signature )
        {
            ...
            (void)writer.WriteLengthPrefixedBytes( gsl::span<const uint8_t>(
                reinterpret_cast<const uint8_t *>( fingerprint.data() ), fingerprint.size() ) );
            (void)writer.WriteLengthPrefixedBytes( signature );
            return writer.Take();
        }
        ...
        std::optional<GenesisRecord> DecodeGenesisRecord( const base::Buffer &value )
        {
            ...
            if ( !domain || !std::equal( domain->begin(), domain->end(), GENESIS_RECORD.begin() ) || !bytes ||
                 ...
                 !reader.Exhausted() ) return std::nullopt;
            GenesisRecord record{ std::move( *bytes ), std::string( fingerprint->begin(), fingerprint->end() ),
                                  std::move( *signature ) };
            if ( !IsHash( record.fingerprint ) ) return std::nullopt;
            return record;
        }
        ...
        std::optional<SignedRecord> DecodeSignedRecord( const base::Buffer &value,
                                                         std::string_view domain )
        {
            ...
            if ( !decoded_domain || !std::equal( decoded_domain->begin(), decoded_domain->end(), domain.begin() ) ||
                 !bytes ) return std::nullopt;
            auto proof = DecodeProof( reader );
            if ( !proof || !reader.Exhausted() ) return std::nullopt;
            return SignedRecord{ std::move( *bytes ), std::move( *proof ) };
        }
    ...
    std::optional<std::vector<uint8_t>> ConfirmedBurnState::CanonicalBytes() const
    {
        if ( encoding_version != ENCODING_VERSION || version == 0 || !IsHash( expected_previous_hash ) ||
             !IsHash( authorizing_policy_hash ) || basis_points > 10000 ) return std::nullopt;
        CanonicalTrustCodec::Writer writer;
    ...
    std::optional<ConfirmedBurnState> ConfirmedBurnState::DecodeCanonical( const std::vector<uint8_t> &bytes )
    {
        ...
        if ( !domain || !std::equal( domain->begin(), domain->end(), BURN_DOMAIN.begin() ) || !encoding ||
             !network || !version_value || !previous || !authorizer || !value || !reader.Exhausted() )
            return std::nullopt;
        ...
        if ( !canonical || *canonical != bytes ) return std::nullopt;
        return burn;
    }
    ...
    outcome::result<ConfirmedTrustSnapshot> TrustStateStore::LoadAndVerify() const
    {
        ...
        if ( genesis_value.has_error() )
        {
            ...
            if ( any_network.has_value() && !any_network.value().empty() ) return outcome::failure( Error::NETWORK_MISMATCH );
            return outcome::failure( Error::NOT_FOUND );
        }
        auto genesis_record = DecodeGenesisRecord( genesis_value.value() );
        if ( !genesis_record ) return outcome::failure( Error::CORRUPT_GENESIS );
        auto genesis = GenesisManifest::DecodeAndVerify( genesis_record->bytes, genesis_record->fingerprint );
        if ( !genesis ) return outcome::failure( Error::CORRUPT_FINGERPRINT );
        if ( genesis->network_id != network_id_ ) return outcome::failure( Error::NETWORK_MISMATCH );
        ...
        if ( policy_head_value.has_error() ) return outcome::failure( Error::MISSING_POLICY_RECORD );
        auto policy_head = DecodeHead( policy_head_value.value() );
        if ( !policy_head ) return outcome::failure( Error::CORRUPT_POLICY_RECORD );
        ...
        for ( uint64_t version = policy_head->first; version > 0; --version )
        {
            ...
            if ( value.has_error() ) return outcome::failure( Error::MISSING_POLICY_RECORD );
            auto record = DecodeSignedRecord( value.value(), POLICY_RECORD );
            if ( !record ) return outcome::failure( Error::CORRUPT_POLICY_RECORD );
        ...
        if ( current_policy == policies.end() ) return outcome::failure( Error::CORRUPT_POLICY_RECORD );
        for ( const auto &[hash, policy] : policies )
        {
            ...
            else
            {
                ...
                if ( predecessor == policies.end() || policy.authorizing_policy_hash != predecessor->first ||
                     ...
                     !VerifyProof( predecessor->second, predecessor->second.membership_threshold, proof, bytes ) )
                    return outcome::failure( Error::INVALID_POLICY_PROOF );
            }
        }

        auto burn_head_value = database_->get( Buffer( BurnHeadKey( network_id_ ) ) );
        if ( burn_head_value.has_error() ) return outcome::failure( Error::MISSING_BURN_RECORD );
        auto burn_head = DecodeHead( burn_head_value.value() );
        if ( !burn_head ) return outcome::failure( Error::CORRUPT_BURN_RECORD );
        ...
        for ( uint64_t version = burn_head->first; version > 0; --version )
        {
            ...
            if ( value.has_error() ) return outcome::failure( Error::MISSING_BURN_RECORD );
            auto record = DecodeSignedRecord( value.value(), BURN_RECORD );
            if ( !record ) return outcome::failure( Error::CORRUPT_BURN_RECORD );
        ...
        if ( current_burn == burns.end() ) return outcome::failure( Error::CORRUPT_BURN_RECORD );
        for ( const auto &[hash, burn] : burns )
        {
            ...
            if ( burn.version == 1 )
            {
                ...
                if ( burn.expected_previous_hash != genesis_record->fingerprint || policy == policies.end() ||
                     policy->second.version != 1 || burn.basis_points != genesis->initial_burn_basis_points ||
                     proof.size() != 1 || proof.front().first != genesis->bootstrapper_public_key ||
                     proof.front().second != genesis_record->signature )
            ...
            else
            {
                ...
                if ( predecessor == burns.end() || predecessor->second.version + 1 != burn.version ||
                     policy == policies.end() ||
                     !VerifyProof( policy->second, policy->second.burn_threshold, proof, bytes ) )
                    return outcome::failure( Error::INVALID_BURN_PROOF );
            }
        }

        return ConfirmedTrustSnapshot{ *genesis, genesis_record->fingerprint, genesis_record->signature,
                                       current_policy->second, policy_proofs.at( current_policy->first ),
                                       current_burn->second, burn_proofs.at( current_burn->first ) };
    }
        ...
        const GenesisManifest &manifest, const std::vector<uint8_t> &bootstrap_signature )
    {
        ...
        if ( database_->contains( Buffer( GenesisKey( network_id_ ) ) ) ) return outcome::failure( Error::ALREADY_INITIALIZED );
        auto canonical = manifest.Canonicalized();
        auto bytes = manifest.CanonicalBytes();
        auto fingerprint = manifest.Fingerprint();
        if ( !canonical || !bytes || !fingerprint ) return outcome::failure( Error::CORRUPT_GENESIS );
        if ( canonical->network_id != network_id_ ) return outcome::failure( Error::NETWORK_MISMATCH );
        ...
        if ( !policy_bytes || !policy_hash ) return outcome::failure( Error::CORRUPT_POLICY_RECORD );

        ConfirmedBurnState burn;
        burn.network_id = network_id_;
        burn.version = 1;
        burn.expected_previous_hash = *fingerprint;
        ...
        if ( !burn_bytes || !burn_hash ) return outcome::failure( Error::CORRUPT_BURN_RECORD );
        multisig::CollectedSignatures proof{ { canonical->bootstrapper_public_key, bootstrap_signature } };

        std::vector<Write> writes{
            { Buffer( GenesisKey( network_id_ ) ), Buffer( EncodeGenesisRecord( *bytes, *fingerprint, bootstrap_signature ) ) },
            { Buffer( PolicyRecordKey( network_id_, 1, *policy_hash ) ), Buffer( EncodeSignedRecord( POLICY_RECORD, *policy_bytes, proof ) ) },
            { Buffer( PolicyHeadKey( network_id_ ) ), Buffer( EncodeHead( 1, *policy_hash ) ) },
            { Buffer( BurnRecordKey( network_id_, 1, *burn_hash ) ), Buffer( EncodeSignedRecord( BURN_RECORD, *burn_bytes, proof ) ) },
            { Buffer( BurnHeadKey( network_id_ ) ), Buffer( EncodeHead( 1, *burn_hash ) ) },
        };
        auto committed = CommitWrites( writes );
        if ( committed.has_error() ) return outcome::failure( Error::COMMIT_FAILED );
        ...
        const QuorumPolicyState &candidate, const multisig::CollectedSignatures &proof )
    {
        ...
        if ( candidate.expected_previous_hash != current_hash ) return outcome::failure( Error::WRONG_PREDECESSOR );
        if ( candidate.authorizing_policy_hash != current_hash ) return outcome::failure( Error::WRONG_AUTHORIZER );
        ...
        if ( !canonical || !bytes || !hash || !ValidatePolicySuccessor( current.policy, candidate ) )
            return outcome::failure( Error::CORRUPT_POLICY_RECORD );
        if ( !VerifyProof( current.policy, current.policy.membership_threshold, proof, *bytes ) )
            return outcome::failure( Error::INVALID_POLICY_PROOF );
        std::vector<Write> writes{
            { Buffer( PolicyRecordKey( network_id_, canonical->version, *hash ) ),
              Buffer( EncodeSignedRecord( POLICY_RECORD, *bytes, proof ) ) },
        ...
        const ConfirmedBurnState &candidate, const multisig::CollectedSignatures &proof )
    {
        ...
        if ( !bytes || !hash || candidate.network_id != network_id_ )
            return outcome::failure( Error::CORRUPT_BURN_RECORD );
        if ( !VerifyProof( current.policy, current.policy.burn_threshold, proof, *bytes ) )
            return outcome::failure( Error::INVALID_BURN_PROOF );
        std::vector<Write> writes{
            { Buffer( BurnRecordKey( network_id_, candidate.version, *hash ) ),
              Buffer( EncodeSignedRecord( BURN_RECORD, *bytes, proof ) ) },
    ...
    {
        if ( committer_ ) return committer_( *database_, writes );
        auto batch = database_->batch();
        if ( !batch ) return outcome::failure( Error::COMMIT_FAILED );
        for ( const auto &[key, value] : writes )
        {
            auto put = batch->put( key, value );
            if ( put.has_error() ) return outcome::failure( Error::COMMIT_FAILED );
        }
        auto commit = batch->commit();
        if ( commit.has_error() ) return outcome::failure( Error::COMMIT_FAILED );
