...
namespace sgns::crypto::bip39 {
  ...
  outcome::result<uint8_t> EntropyAccumulator::calculateChecksum() const {
    BOOST_OUTCOME_TRY((auto &&, entropy), getEntropy());
...
OUTCOME_CPP_DEFINE_CATEGORY_3(sgns::crypto::bip39, Bip39EntropyError, error) {
  using E = sgns::crypto::bip39::Bip39EntropyError;
  switch (error) {
