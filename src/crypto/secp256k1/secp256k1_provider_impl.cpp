...
namespace sgns::crypto
{
    ...
    outcome::result<secp256k1::UncompressedPublicKey> Secp256k1ProviderImpl::recoverPublickeyUncompressed(
        const secp256k1::RSVSignature &signature,
        const secp256k1::MessageHash  &message_hash ) const
    {
        BOOST_OUTCOME_TRY( auto pubkey, recoverPublickey( signature, message_hash ) );
        secp256k1::UncompressedPublicKey pubkey_out;
        size_t                           outputlen = pubkey_out.size();

        if ( 1 != secp256k1_ec_pubkey_serialize( context_.get(),
                                                 pubkey_out.data(),
                                                 &outputlen,
                                                 &pubkey,
    ...
    outcome::result<secp256k1::CompressedPublicKey> Secp256k1ProviderImpl::recoverPublickeyCompressed(
        const secp256k1::RSVSignature &signature,
        const secp256k1::MessageHash  &message_hash ) const
    {
        BOOST_OUTCOME_TRY( auto pubkey, recoverPublickey( signature, message_hash ) );
    ...
    outcome::result<secp256k1_pubkey> Secp256k1ProviderImpl::recoverPublickey(
        ...
        const secp256k1::MessageHash  &message_hash ) const
    {
        BOOST_OUTCOME_TRY( auto rec_id, validateRecoveryId( static_cast<int>( signature[64] ) ) );

        secp256k1_ecdsa_recoverable_signature sig_rec;
        secp256k1_pubkey                      pubkey;

        if ( 1 !=
             secp256k1_ecdsa_recoverable_signature_parse_compact( context_.get(), &sig_rec, signature.data(), rec_id ) )
        {
            return Secp256k1ProviderError::INVALID_R_OR_S_VALUE;
        }

        if ( 1 != secp256k1_ecdsa_recover( context_.get(), &pubkey, &sig_rec, message_hash.data() ) )
        {
...
{
    using E = sgns::crypto::Secp256k1ProviderError;
    switch ( e )
    {
