...
namespace sgns::subscription
{
   ...
   */
    ...
    class Subscriber final : public std::enable_shared_from_this<Subscriber<Key, Type, Arguments...>>
    {
    public:
        using KeyType           = Key;
        ...
        using SubscriptionEngineType = SubscriptionEngine<KeyType, ValueType, Arguments...>;
    ...
    private:
        using SubscriptionsContainer = std::unordered_map<KeyType, typename SubscriptionEngineType::IteratorType>;
    ...
    public:
        ...
        void subscribe( SubscriptionSetId id, const KeyType &key )
        {
            ...
            auto &&[it, inserted] = subscriptions_sets_[id].emplace( key,
        ...
        void on_notify( const KeyType &key, const Arguments &...args )
        {
            ...
            {
                on_notify_callback_( object_, key, args... );
            }
        }
    };
