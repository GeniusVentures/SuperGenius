...
BridgeRpcWatcher::BridgeRpcWatcher(
    const Config       &config,
...
void BridgeRpcWatcher::startWatching()
{
    ...
    logger->info("BridgeRpcWatcher starting: rpc={}, chain_id={}, contract={}, event={}",
                 config_.rpc_url, config_.chain_id,
                 config_.contract_address, config_.event_signature);
...
void BridgeRpcWatcher::watch()
{
    while (running)
    {
        ...
        boost::this_thread::sleep_for(
            boost::chrono::seconds(config_.poll_interval.count()));
    }
}
...
bool BridgeRpcWatcher::poll_once()
{
    ...
    // 1. Get latest block
    const auto latest_block_req = eth::rpc::make_get_block_by_number_request(
        eth::rpc::RpcBlockTag::kLatest, 1);
    ...
    if (last_block_ == 0)
    {
        last_block_ = (safe_latest > config_.max_log_range)
                          ? (safe_latest - config_.max_log_range + 1)
                          : 0ULL;
    }
    ...
    const auto get_logs_req = eth::rpc::make_get_logs_request(
        filter, last_block_, to_block, 2);
    ...
    {
        const auto receipt_req = eth::rpc::make_get_transaction_receipt_request(
            rpc_log.tx_hash, 3);
        ...
        const auto receipt = eth::rpc::parse_transaction_receipt_response(
            receipt_resp.value());
