...
namespace sgns
{
    ...
    outcome::result<std::set<uint64_t>> TransactionManager::SendTransactionItem( TransactionItem &item )
    {
        ...
        {
            BOOST_OUTCOME_TRY( auto &&parsedTopics, ParseTransaction( tx ) );
            topicSet.insert( parsedTopics.begin(), parsedTopics.end() );
            {
                ...
                {
                    outgoing_tx_processed_m[key]   = TrackedTx{ tx, tx_state, nonce };
                    outgoing_nonce_to_key_m[nonce] = key; // Add to nonce index
    ...
    outcome::result<void> TransactionManager::RollbackTransactions( TransactionItem &item_to_rollback )
    {
        ...
        for ( auto &[transaction, __dontcare] : transaction_batch )
        {
            ...
            }
            {
                ...
                if ( auto it = outgoing_tx_processed_m.find( key ); it != outgoing_tx_processed_m.end() )
                {
    ...
    outcome::result<void> TransactionManager::CheckIncoming()
    {
        ...
        for ( auto network_id : monitored_networks )
        {
            ...
            m_logger->trace( "[{} - full: {}] Probing incoming transactions on {}",
                             ...
                             full_node_m,
                             query_path );
            BOOST_OUTCOME_TRY( ( auto &&, transaction_list ),
            ...
            for ( const auto &[key, _] : transaction_list )
            {
...

                {
                    m_logger->trace( "[{} - full: {}] Inserting into incoming {}",
                                     ...
                                     full_node_m,
                                     transaction_key.value() );
                    std::unique_lock out_lock( incoming_tx_mutex_m );
    ...
    void TransactionManager::InitNonce( uint64_t timeout_ms )
    {
        ...
        if ( nonce_result.has_value() )
        {
            ...
            {
                if ( auto tx = GetOutTransaction( i, account_m->GetAddress() ); !tx )
                {
                    synced = false;
    ...
    outcome::result<bool> TransactionManager::CheckTransactionValidity( const std::set<uint64_t> &nonces_to_check )
    {
        ...
        std::vector<std::string> invalid_transaction_keys;
        {
            ...
            {
                for ( auto [_, tracked] : outgoing_tx_processed_m )
                {
                    if ( !tracked.tx )
                    {
                        continue;
                    ...
                    if ( tracked.tx->dag_st.nonce() == nonce )
                    {
                        ...
                        if ( !valid_tx )
                        {
                            ...
                            invalid_transaction_keys.push_back( _ );
                            changed = true;
    ...
    std::shared_ptr<IGeniusTransactions> TransactionManager::GetOutTransaction( const std::string &tx_hash ) const
    {
        for ( const auto &[_, tracked] : outgoing_tx_processed_m )
        {
            m_logger->debug( "[{} - full: {}] Searching for hash {}",
                             ...
                             full_node_m,
                             tx_hash );
            if ( tracked.tx && tracked.tx->dag_st.data_hash() == tx_hash )
            {
                return tracked.tx;
            }
        }
    ...
    outcome::result<void> TransactionManager::ConfirmTransactions()
    {
        ...
        {
            std::shared_lock out_lock( outgoing_tx_mutex_m );
            // Use nonce index to check for VERIFYING transactions
            for ( const auto &[nonce, key] : outgoing_nonce_to_key_m )
            {
                auto it = outgoing_tx_processed_m.find( key );
                if ( it != outgoing_tx_processed_m.end() && it->second.status == TransactionStatus::VERIFYING )
                {
                    verifying_nonces.push_back( nonce );
                }
            }
        }
                                                                                  ...
                                                                                  bool               delete_from_crdt )
    {
        ...
        // Check and remove from outgoing_tx_processed_m
        {
            ...
            if ( out_it != outgoing_tx_processed_m.end() )
            {
                ...
                if ( out_it->second.tx )
                {
                    BOOST_OUTCOME_TRY( auto &&topics, RevertTransaction( out_it->second.tx ) );
                    if ( delete_from_crdt )
                    {
                        BOOST_OUTCOME_TRY( DeleteTransaction( out_it->first, topics ) );
                    }
                    account_m->RollBackPeerConfirmedNonce( out_it->second.tx->dag_st.nonce(),
                                                           out_it->second.tx->dag_st.source_addr() );
                    // Update verifying_count if this was VERIFYING
                    if ( out_it->second.status == TransactionStatus::VERIFYING )
                    {
                        verifying_count_.fetch_sub( 1, std::memory_order_relaxed );
                    }
                    // Remove from nonce index
                    outgoing_nonce_to_key_m.erase( out_it->second.tx->dag_st.nonce() );
                }
        ...
        // Check and remove from incoming_tx_processed_m
        {
            ...
            if ( in_it != incoming_tx_processed_m.end() )
            {
                ...
                if ( in_it->second.tx )
                {
                    BOOST_OUTCOME_TRY( auto &&topics, RevertTransaction( in_it->second.tx ) );
                    if ( delete_from_crdt )
                    {
                        BOOST_OUTCOME_TRY( DeleteTransaction( in_it->first, topics ) );
                    }
        ...
        crdt::CRDTCallbackManager::NewDataPair new_data )
    {
        ...
        if ( new_tx->GetSrcAddress() == account_m->GetAddress() )
        {
            ...
            {
                BOOST_OUTCOME_TRY( auto &&topics, RevertTransaction( it->second.tx ) );
            ...
            else if ( it == outgoing_tx_processed_m.end() )
            {
                ...
                if ( conflicting_tx.has_value() )
                {
                    ...
                    for ( const auto &[key, tracked] : outgoing_tx_processed_m )
                    {
                        if ( tracked.tx &&
                             tracked.tx->dag_st.data_hash() == conflicting_tx.value()->dag_st.data_hash() )
                        {
                            conflicting_key = key;
                            break;
                        }
                    ...
                    if ( !conflicting_key.empty() )
                    {
                        ...
                        out_lock.unlock();
                        BOOST_OUTCOME_TRY( RemoveTransactionFromProcessedMaps( conflicting_key, true ) );
                        out_lock.lock();
                    }
                    else
                    {
        ...
        else
        {
            ...
            else if ( it == incoming_tx_processed_m.end() )
            {
                ...
                if ( conflicting_tx.has_value() )
                {
                    // Find the key for the conflicting transaction
                    std::string conflicting_key;
                    for ( const auto &[key, tracked] : incoming_tx_processed_m )
                    {
                        if ( tracked.tx &&
                             tracked.tx->dag_st.data_hash() == conflicting_tx.value()->dag_st.data_hash() )
                        {
        ...
        const IGeniusTransactions &element ) const
    {
        ...
        {
            if ( auto tx = GetInTransaction( element.dag_st.nonce(), element.GetSrcAddress() ) )
            {
                return tx;
            }
        }
        ...
        {
            if ( auto tx = GetOutTransaction( element.dag_st.nonce(), element.GetSrcAddress() ) )
            {
                return tx;
            }
        }
