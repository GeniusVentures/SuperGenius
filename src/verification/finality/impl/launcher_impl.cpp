...
namespace sgns::verification::finality {
  ...
  void LauncherImpl::executeNextRound() {
    ...
    BOOST_ASSERT_MSG(
        ...
        "Voters does not exist in storage. Stopping finality execution");
    const auto &voters = voters_res.value();
    BOOST_ASSERT_MSG(voters->size() != 0,
                     "Voters are empty. Stopping finality execution");
    ...
    FinalityConfig config{/*.voters =*/ voters,
    ...
    current_round_ =
        std::make_shared<VotingRoundImpl>(std::move(config),
                                          ...
                                          clock_,
                                          io_context_);
    logger_->debug("Starting finality round: {}", round_number);
  ...
  void LauncherImpl::startLivenessChecker() {
    // check if round id was updated. If it was not, that means that finality is
    // not working
