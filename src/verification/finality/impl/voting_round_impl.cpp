...
namespace sgns::verification::finality {
  ...
  void VotingRoundImpl::endWaitingStage() {
    ...
    // Play new round
    // spec: Play-Finality-round(r + 1);

    if (auto finality = finality_.lock()) {
...
//		// spec: Broadcast(M vr;pv (N ))
  ...
  bool VotingRoundImpl::tryFinalize() {
    ...
    if (auto notify_res = notify(*previous_round_state_); ! notify_res) {
