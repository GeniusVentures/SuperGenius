...
namespace sgns::authority {
  ...
  AuthorityManagerImpl::AuthorityManagerImpl(
      ...
      : log_{base::createLogger("AuthMngr")},
  ...
  void AuthorityManagerImpl::stop() {
    ...
    auto save_res =
        storage_->put(SCHEDULER_TREE, base::Buffer(data_res.value()));
  ...
  outcome::result<void> AuthorityManagerImpl::onVerification(
      const primitives::VerificationEngineId &engine_id,
      const primitives::BlockInfo &block,
      const primitives::Verification &message) {
