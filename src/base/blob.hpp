...
namespace sgns::base
{
   ...
   */
     ...
     */
        static outcome::result<Blob<size_>> fromSpan( gsl::span<uint8_t> span )
        {
            if ( span.size() != size_ )
            {
                return BlobError::INCORRECT_LENGTH;
   ...
   */
    template <class Stream, size_t size, typename = std::enable_if_t<Stream::is_encoder_stream>>
   ...
   * @return reference to stream
   */
    template <class Stream, size_t size, typename = std::enable_if_t<Stream::is_decoder_stream>>
