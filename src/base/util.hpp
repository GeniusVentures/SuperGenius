...
namespace sgns
{
     ...
     */
    static outcome::result<uint64_t> String2Fixed( const std::string &str_value, uint64_t precision = 9 )
    {
        if ( str_value.empty() )
        {
     ...
     */
    static std::string Fixed2String( uint64_t value, uint64_t precision = 9 )
    {
        uint64_t scale = 1;
        for ( uint64_t i = 0; i < precision; ++i )
        {
     ...
     */
    static uint64_t FixedPointMultiply( uint64_t a, uint64_t b, uint64_t precision = 9 )
    {
        uint64_t scale = 1;
        for ( uint64_t i = 0; i < precision; ++i )
        {
     ...
     */
    static outcome::result<uint64_t> FixedPointDivide( uint64_t a, uint64_t b, uint64_t precision = 9 )
    {
        if ( b == 0 )
        {
