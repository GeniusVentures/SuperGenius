...
namespace sgns::api
{
        ...
        secure( false )
    {
        ...
        else
        {
            ...
            stream.emplace<PlainWebSocketStream>(
                boost::beast::websocket::stream<boost::asio::basic_stream_socket<boost::asio::ip::tcp>>(
                    make_strand( context ) ) );
        }
    }
                                ...
                                [self, this, &streamVariant]( boost::system::error_code ec, const auto & )
                                {
                                    ...
                                    streamVariant.next_layer().async_handshake(
                                        ...
                                        [self, this]( boost::system::error_code ec )
                                        {
                                            ...
                                            if ( !stream.has_value() )
                                            {
                                                self->reportError( boost::asio::error::not_connected,
                                                                   "Stream is not initialized" );
                                                return;
                                            }

                                            std::visit(
                                                [self]( auto &innerStreamVariant )
                                                {
                                                    innerStreamVariant.async_handshake(
                                                        self->host,
                                                        "/",
                                                        [self]( boost::system::error_code ec )
                                                        {
                                                            ...
                                                            std::cout << "Successfully connected to WebSocket server."
                                                                      << std::endl;
                                                        } );
                                                },
                                                stream.value() );
    ...
    void WsClientImpl::setMessageHandler( std::function<void( const std::string & )> handler )
    {
        ...
        std::visit(
            [self, handler, this]( auto &streamVariant )
            {
                streamVariant.async_read( buffer,
                                          [self, handler]( boost::system::error_code ec, std::size_t bytes_transferred )
                                          {
                                              ...
                                              {
                                                  std::string message(
                                                      boost::asio::buffer_cast<const char *>( self->buffer.data() ),
                                                      bytes_transferred );
                                                  handler( message );
    ...
    void WsClientImpl::send( const std::string &message )
    {
        ...
        std::visit(
            [self, message]( auto &streamVariant )
            {
