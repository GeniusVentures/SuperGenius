...
namespace {
  thread_local class {
    boost::optional<sgns::api::Session::SessionId> bound_session_id_ =
        boost::none;

   public:
    void storeSessionId(sgns::api::Session::SessionId id) {
      bound_session_id_ = id;
    }
    void releaseSessionId() {
      bound_session_id_ = boost::none;
    }
    boost::optional<sgns::api::Session::SessionId> fetchSessionId() {
        ...
        if (SessionType::kWs == session->type()) {
          ...
          subscribed_session->setCallback([wp](SessionPtr &session,
            ...
            if (auto self = wp.lock()) {
              ...
              /// TODO: PRE-475 make event notofication depending in
