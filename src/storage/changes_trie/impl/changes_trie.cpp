        ...
        const ChangesTrieConfig                                      &config )
    {
        ...
        for ( auto &change : extinsics_changes )
        {
            ...
            BOOST_OUTCOME_TRYV2( auto &&,
                                 changes_storage->put( base::Buffer{ std::move( key_enc ) }, std::move( value_buf ) ) );
        }

        return std::unique_ptr<ChangesTrie>{ new ChangesTrie{ std::move( changes_storage ), std::move( codec ) } };
    }

    ChangesTrie::ChangesTrie( std::unique_ptr<storage::trie::SuperGeniusTrie> trie,
                              std::shared_ptr<storage::trie::Codec>           codec ) :
        changes_trie_{ std::move( trie ) }, codec_{ std::move( codec ) }, logger_( base::createLogger( "ChangesTrie" ) )
    {
