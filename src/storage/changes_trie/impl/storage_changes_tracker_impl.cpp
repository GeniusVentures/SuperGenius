...
namespace sgns::storage::changes_trie
{
    ...
    outcome::result<void> StorageChangesTrackerImpl::onBlockChange( primitives::BlockHash   new_parent_hash,
    ...
    void StorageChangesTrackerImpl::setExtrinsicIdxGetter( GetExtrinsicIndexDelegate f )
    {
        get_extrinsic_index_ = std::move( f );
    }

    outcome::result<void> StorageChangesTrackerImpl::onPut( const base::Buffer &key,
                                                            const base::Buffer &value,
                                                            bool                is_new_entry )
    {
        auto change_it = extrinsics_changes_.find( key );
        BOOST_OUTCOME_TRY( auto idx_bytes, get_extrinsic_index_() );
        BOOST_OUTCOME_TRY( auto idx, scale::decode<primitives::ExtrinsicIndex>( idx_bytes ) );
    ...
    outcome::result<void> StorageChangesTrackerImpl::onRemove( const base::Buffer &key )
    {
        auto change_it = extrinsics_changes_.find( key );
        BOOST_OUTCOME_TRY( auto idx_bytes, get_extrinsic_index_() );
        BOOST_OUTCOME_TRY( auto idx, scale::decode<primitives::ExtrinsicIndex>( idx_bytes ) );
    ...
    outcome::result<base::Hash256> StorageChangesTrackerImpl::constructChangesTrie( const primitives::BlockHash &parent,
                                                                                    const ChangesTrieConfig     &conf )
    {
        ...
        BOOST_OUTCOME_TRY(
            ( auto &&, trie ),
            ChangesTrie::buildFromChanges( parent_number_, trie_factory_, codec_, extrinsics_changes_, conf ) );
        return trie->getHash();
    }
