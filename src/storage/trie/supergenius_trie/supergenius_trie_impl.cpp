...
{
    SuperGeniusTrieImpl::SuperGeniusTrieImpl( ChildRetrieveFunctor f ) : retrieve_child_{ std::move( f ) }
    {
    ...
    outcome::result<void> SuperGeniusTrieImpl::put( const Buffer &key, const Buffer &value )
    {
    ...
    outcome::result<void> SuperGeniusTrieImpl::put( const Buffer &key, Buffer &&value )
    {
        ...
        BOOST_OUTCOME_TRY( auto n, insert( root, k_enc, std::make_shared<LeafNode>( k_enc, value ) ) );
        root_ = n;

        return outcome::success();
    }

    outcome::result<void> SuperGeniusTrieImpl::clearPrefix( const base::Buffer &prefix )
    {
        if ( !root_ )
        {
            return outcome::success();
        ...
        BOOST_OUTCOME_TRY( auto new_root, detachNode( root_, key_nibbles ) );
        root_ = new_root;

        return outcome::success();
    }

    outcome::result<SuperGeniusTrie::NodePtr> SuperGeniusTrieImpl::insert( const NodePtr    &parent,
    ...
    outcome::result<SuperGeniusTrie::NodePtr> SuperGeniusTrieImpl::updateBranch( BranchPtr         parent,
                                                                                 const KeyNibbles &key_nibbles,
                                                                                 const NodePtr    &node )
    {
        auto length = getCommonPrefixLength( key_nibbles, parent->key_nibbles );
        ...
        if ( length == parent->key_nibbles.size() )
        {
            ...
            BOOST_OUTCOME_TRY( auto child, retrieveChild( parent, key_nibbles[length] ) );
            if ( child )
            {
                BOOST_OUTCOME_TRY( auto n, insert( child, key_nibbles.subspan( length + 1 ), node ) );
        ...
        BOOST_OUTCOME_TRY( auto new_branch, insert( nullptr, parent->key_nibbles.subspan( length + 1 ), parent ) );
        ...
        {
            BOOST_OUTCOME_TRY( auto new_child, insert( nullptr, key_nibbles.subspan( length + 1 ), node ) );
            br->children.at( key_nibbles[length] ) = new_child;
        }
        return br;
    }

    outcome::result<base::Buffer> SuperGeniusTrieImpl::get( const base::Buffer &key ) const
    {
        if ( !root_ )
        {
        ...
        BOOST_OUTCOME_TRY( auto node, getNode( root_, nibbles ) );
        if ( node && node->value )
        {
    ...
    outcome::result<SuperGeniusTrie::NodePtr> SuperGeniusTrieImpl::getNode( NodePtr           parent,
                                                                            const KeyNibbles &key_nibbles ) const
    {
        ...
        switch ( parent->getTrieType() )
        {
            ...
            case T::BranchWithValue:
            {
                ...
                auto parent_as_branch = std::dynamic_pointer_cast<BranchNode>( parent );
                BOOST_OUTCOME_TRY( auto n, retrieveChild( parent_as_branch, key_nibbles[length] ) );
    ...
    outcome::result<std::list<std::pair<SuperGeniusTrieImpl::BranchPtr, uint8_t>>> SuperGeniusTrieImpl::getPath(
        NodePtr           parent,
        const KeyNibbles &key_nibbles ) const
    {
        ...
        switch ( parent->getTrieType() )
        {
            ...
            case T::BranchWithValue:
            {
                ...
                auto parent_as_branch = std::dynamic_pointer_cast<BranchNode>( parent );
                BOOST_OUTCOME_TRY( auto n, retrieveChild( parent_as_branch, key_nibbles[length] ) );
                BOOST_OUTCOME_TRY( auto path, getPath( n, key_nibbles.subspan( length + 1 ) ) );
    ...
    bool SuperGeniusTrieImpl::contains( const base::Buffer &key ) const
    {
        ...
        return node.has_value() && ( node.value() != nullptr ) && ( node.value()->value );
    }
    ...
    outcome::result<void> SuperGeniusTrieImpl::remove( const base::Buffer &key )
    {
        if ( root_ )
        {
            ...
            BOOST_OUTCOME_TRY( auto n, deleteNode( root_, key_nibbles ) );
    ...
    outcome::result<SuperGeniusTrie::NodePtr> SuperGeniusTrieImpl::deleteNode( NodePtr           parent,
                                                                               const KeyNibbles &key_nibbles )
    {
        if ( !parent )
        {
        ...
        switch ( parent->getTrieType() )
        {
            ...
            case T::BranchEmptyValue:
            {
                ...
                {
                    BOOST_OUTCOME_TRY( auto child, retrieveChild( parent_as_branch, key_nibbles[length] ) );
                    BOOST_OUTCOME_TRY( auto n, deleteNode( child, key_nibbles.subspan( length + 1 ) ) );
                    newRoot                                              = parent;
                    parent_as_branch->children.at( key_nibbles[length] ) = n;
                }
                BOOST_OUTCOME_TRY( auto n, handleDeletion( parent_as_branch, newRoot, key_nibbles ) );
                return n;
            }
            case T::Leaf:
                if ( parent->key_nibbles == key_nibbles || key_nibbles.empty() )
    ...
    outcome::result<SuperGeniusTrie::NodePtr> SuperGeniusTrieImpl::handleDeletion( const BranchPtr  &parent,
                                                                                   NodePtr           node,
                                                                                   const KeyNibbles &key_nibbles )
    {
        ...
        {
            newRoot = std::make_shared<LeafNode>( key_nibbles.subspan( 0, length ), parent->value );
        }
        else if ( parent->childrenNum() == 1 && !parent->value )
        {
            ...
            BOOST_OUTCOME_TRY( auto child, retrieveChild( parent, idx ) );
            using T = SuperGeniusNode::Type;
            ...
            else if ( child->getTrieType() == T::BranchEmptyValue || child->getTrieType() == T::BranchWithValue )
            {
                auto branch = std::make_shared<BranchNode>();
                branch->key_nibbles.putBuffer( parent->key_nibbles ).putUint8( idx ).putBuffer( child->key_nibbles );
    ...
    outcome::result<SuperGeniusTrie::NodePtr> SuperGeniusTrieImpl::detachNode( const NodePtr    &parent,
                                                                               const KeyNibbles &prefix_nibbles )
    {
        ...
        if ( parent->key_nibbles.size() >= prefix_nibbles.size() )
        {
            ...
            if ( parent->key_nibbles.subbuffer( 0, prefix_nibbles.size() ) == prefix_nibbles )
            {
                return nullptr;
            }
            return parent;
        ...
        // change anything
        if ( prefix_nibbles.subbuffer( 0, parent->key_nibbles.size() ) != parent->key_nibbles )
        {
            return parent;
        }
        using T = SuperGeniusNode::Type;
        if ( parent->getTrieType() == T::BranchWithValue || parent->getTrieType() == T::BranchEmptyValue )
        {
            ...
            BOOST_OUTCOME_TRY( auto child, retrieveChild( branch, prefix_nibbles[length] ) );
            if ( child == nullptr )
            {
                return parent;
            }
            BOOST_OUTCOME_TRY( auto n, detachNode( child, prefix_nibbles.subspan( length + 1 ) ) );
    ...
    outcome::result<SuperGeniusTrie::NodePtr> SuperGeniusTrieImpl::retrieveChild( BranchPtr parent, uint8_t idx ) const
    {
        return retrieve_child_( std::move( parent ), idx );
    }

    uint32_t SuperGeniusTrieImpl::getCommonPrefixLength( const KeyNibbles &pref1, const KeyNibbles &pref2 )
    {
        size_t length = 0;
        auto   min    = pref1.size();
