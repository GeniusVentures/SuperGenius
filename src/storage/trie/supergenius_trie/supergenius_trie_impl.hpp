...
namespace sgns::storage::trie
{
    ...
    public:
        using ChildRetrieveFunctor = std::function<outcome::result<NodePtr>( BranchPtr, uint8_t )>;
     ...
     */
        explicit SuperGeniusTrieImpl( ChildRetrieveFunctor f = defaultChildRetrieveFunctor );

        explicit SuperGeniusTrieImpl( NodePtr root, ChildRetrieveFunctor f = defaultChildRetrieveFunctor );

        NodePtr getRoot() const override;

        outcome::result<NodePtr> getNode( NodePtr parent, const KeyNibbles &key_nibbles ) const override;

        outcome::result<std::list<std::pair<BranchPtr, uint8_t>>> getPath(
            NodePtr           parent,
            const KeyNibbles &key_nibbles ) const override;
     ...
     */
        ...
        outcome::result<void> put( const base::Buffer &key, const base::Buffer &value ) override;
        ...
        outcome::result<base::Buffer> get( const base::Buffer &key ) const override;
    ...
    private:
        outcome::result<NodePtr> insert( const NodePtr &parent, const KeyNibbles &key_nibbles, NodePtr node );

        outcome::result<NodePtr> updateBranch( BranchPtr parent, const KeyNibbles &key_nibbles, const NodePtr &node );

        outcome::result<NodePtr> deleteNode( NodePtr parent, const KeyNibbles &key_nibbles );
        outcome::result<NodePtr> handleDeletion( const BranchPtr &parent, NodePtr node, const KeyNibbles &key_nibbles );
        // remove a node with its children
        outcome::result<NodePtr> detachNode( const NodePtr &parent, const KeyNibbles &prefix_nibbles );

        static uint32_t getCommonPrefixLength( const KeyNibbles &pref1, const KeyNibbles &pref2 );

        outcome::result<NodePtr> retrieveChild( BranchPtr parent, uint8_t idx ) const override;
