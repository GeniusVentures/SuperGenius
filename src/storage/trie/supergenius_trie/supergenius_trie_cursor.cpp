...
OUTCOME_CPP_DEFINE_CATEGORY_3( sgns::storage::trie, SuperGeniusTrieCursor::Error, e )
{
...
namespace sgns::storage::trie
{
    ...
    SuperGeniusTrieCursor::SuperGeniusTrieCursor( const SuperGeniusTrie &trie ) : trie_{ trie }, current_{ nullptr } {}

    outcome::result<std::unique_ptr<SuperGeniusTrieCursor>> SuperGeniusTrieCursor::createAt(
        const base::Buffer    &key,
        const SuperGeniusTrie &trie )
    {
        auto c = std::make_unique<SuperGeniusTrieCursor>( trie );
        BOOST_OUTCOME_TRY( auto node, trie.getNode( trie.getRoot(), c->codec_.keyToNibbles( key ) ) );
    ...
    outcome::result<void> SuperGeniusTrieCursor::seekToLast()
    {
        ...
        while ( current->getTrieType() != NodeType::Leaf )
        {
            ...
            if ( type == NodeType::BranchEmptyValue || type == NodeType::BranchWithValue )
            {
    ...
    outcome::result<void> SuperGeniusTrieCursor::next()
    {
        ...
        if ( current_ == nullptr )
        {
            return trie_.getRoot() == nullptr ? Error::NULL_ROOT : Error::INVALID_CURSOR_POSITION;
        }
        ...
        do
        {
                      ...
                      current_->getTrieType() == NodeType::BranchWithValue )
            {
                ...
                if ( last_visited_child_.empty() || last_visited_child_.back().parent != p )
                {
    ...
    base::Buffer SuperGeniusTrieCursor::collectKey() const
    {
        ...
        for ( auto &node_idx : last_visited_child_ )
        {
            ...
            std::copy( node->key_nibbles.begin(), node->key_nibbles.end(), std::back_inserter<Buffer>( key_nibbles ) );
    ...
    int8_t SuperGeniusTrieCursor::getNextChildIdx( const BranchPtr &parent, uint8_t child_idx )
    {
    ...
    bool SuperGeniusTrieCursor::hasNextChild( const BranchPtr &parent, uint8_t child_idx )
    {
        return getNextChildIdx( parent, child_idx ) != -1;
    }

    int8_t SuperGeniusTrieCursor::getPrevChildIdx( const BranchPtr &parent, uint8_t child_idx )
    {
    ...
    bool SuperGeniusTrieCursor::hasPrevChild( const BranchPtr &parent, uint8_t child_idx )
    {
        return getPrevChildIdx( parent, child_idx ) != -1;
    }

    void SuperGeniusTrieCursor::updateLastVisitedChild( const BranchPtr &parent, uint8_t child_idx )
    {
    ...
    auto SuperGeniusTrieCursor::constructLastVisitedChildPath( const base::Buffer &key )
