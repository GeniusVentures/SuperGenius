...
namespace sgns::storage::trie
{
    ...
    class SuperGeniusTrieCursor : public face::MapCursor<base::Buffer, base::Buffer>
    {
    public:
        ...
        enum class Error
        {
            INVALID_CURSOR_POSITION = 1, // operation cannot be performed for cursor position is not valid
        ...
        static outcome::result<std::unique_ptr<SuperGeniusTrieCursor>> createAt( const base::Buffer    &key,
     ...
     */
        ...
        {
            TriePathEntry( BranchPtr parent, int8_t child_idx ) : parent{ std::move( parent ) }, child_idx{ child_idx }
            {
            }
     ...
     */
        auto constructLastVisitedChildPath( const base::Buffer &key ) -> outcome::result<std::list<TriePathEntry>>;
