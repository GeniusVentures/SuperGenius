...
namespace sgns::storage::trie
{
    ...
    const base::Buffer EXTRINSIC_INDEX_KEY = base::Buffer{}.put( ":extrinsic_index" );
    ...
    outcome::result<Buffer> PersistentTrieBatchImpl::get( const Buffer &key ) const
    {
    ...
    outcome::result<void> PersistentTrieBatchImpl::clearPrefix( const Buffer &prefix )
    {
        // TODO(Harrm): notify changes tracker
        return trie_->clearPrefix( prefix );
    }

    outcome::result<void> PersistentTrieBatchImpl::put( const Buffer &key, const Buffer &value )
    {
        return put( key, Buffer{ value } ); // would have to copy anyway
    }

    outcome::result<void> PersistentTrieBatchImpl::put( const Buffer &key, Buffer &&value )
    {
