...
namespace sgns::storage::trie
{
    ...
    TrieStorageBackendImpl::TrieStorageBackendImpl( std::shared_ptr<BufferStorage> storage, base::Buffer node_prefix ) :
    ...
    {
        return storage_->cursor(); // TODO(Harrm): perhaps should iterate over trie nodes only
    }

    std::unique_ptr<face::WriteBatch<Buffer, Buffer>> TrieStorageBackendImpl::batch()
    {
        return std::make_unique<TrieStorageBackendBatch>( storage_->batch(), node_prefix_ );
    }
    ...
    outcome::result<void> TrieStorageBackendImpl::put( const Buffer &key, const Buffer &value )
    {
        return storage_->put( prefixKey( key ), value );
    }

    outcome::result<void> TrieStorageBackendImpl::put( const Buffer &key, Buffer &&value )
    {
