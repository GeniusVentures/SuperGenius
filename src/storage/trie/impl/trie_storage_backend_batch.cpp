...
namespace sgns::storage::trie
{
    ...
    outcome::result<void> TrieStorageBackendBatch::put( const base::Buffer &key, const base::Buffer &value )
    {
        return storage_batch_->put( prefixKey( key ), value );
    }

    outcome::result<void> TrieStorageBackendBatch::put( const base::Buffer &key, base::Buffer &&value )
    {
        return storage_batch_->put( prefixKey( key ), std::move( value ) );
    }

    outcome::result<void> TrieStorageBackendBatch::remove( const base::Buffer &key )
    {
        return storage_batch_->remove( prefixKey( key ) );
    }

    base::Buffer TrieStorageBackendBatch::prefixKey( const base::Buffer &key ) const
    {
