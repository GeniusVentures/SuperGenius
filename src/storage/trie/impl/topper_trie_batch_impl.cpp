...
namespace sgns::storage::trie
{
    ...
    TopperTrieBatchImpl::TopperTrieBatchImpl( const std::shared_ptr<TrieBatch> &parent ) : parent_( parent ) {}
    ...
    {
        if ( !cache_.empty() &&
             std::any_of( cache_.begin(), cache_.end(), []( auto &p ) { return p.second.has_value(); } ) )
        {
            return false;
        }
    ...
    outcome::result<void> TopperTrieBatchImpl::put( const Buffer &key, const Buffer &value )
    {
        return put( key, Buffer( value ) );
    }

    outcome::result<void> TopperTrieBatchImpl::put( const Buffer &key, Buffer &&value )
    {
