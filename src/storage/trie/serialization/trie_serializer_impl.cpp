...
namespace sgns::storage::trie
{
    ...
    TrieSerializerImpl::TrieSerializerImpl( std::shared_ptr<SuperGeniusTrieFactory> factory,
                                            std::shared_ptr<Codec>                  codec,
                                            std::shared_ptr<TrieStorageBackend>     backend ) :
        trie_factory_{ std::move( factory ) }, codec_{ std::move( codec ) }, backend_{ std::move( backend ) }
    {
    ...
    outcome::result<std::unique_ptr<SuperGeniusTrie>> TrieSerializerImpl::retrieveTrie(
        const base::Buffer &db_key ) const
    {
        SuperGeniusTrieFactory::ChildRetrieveFunctor f = [this]( const SuperGeniusTrie::BranchPtr &parent, uint8_t idx )
    ...
    outcome::result<Buffer> TrieSerializerImpl::storeRootNode( SuperGeniusNode &node )
    {
        ...
        if ( node.getTrieType() == T::BranchEmptyValue || node.getTrieType() == T::BranchWithValue )
        {
            auto &branch = dynamic_cast<BranchNode &>( node );
            BOOST_OUTCOME_TRY( storeChildren( branch, *batch ) );
        }

        BOOST_OUTCOME_TRY( auto enc, codec_->encodeNode( node ) );
        auto key = Buffer{ codec_->hash256( enc ) };
        BOOST_OUTCOME_TRY( batch->put( key, enc ) );
        BOOST_OUTCOME_TRY( batch->commit() );

        return key;
    }

    outcome::result<base::Buffer> TrieSerializerImpl::storeNode( SuperGeniusNode &node, BufferBatch &batch )
    {
        using T = SuperGeniusNode::Type;
    ...
    outcome::result<void> TrieSerializerImpl::storeChildren( BranchNode &branch, BufferBatch &batch )
    {
    ...
    outcome::result<SuperGeniusTrie::NodePtr> TrieSerializerImpl::retrieveChild(
        const SuperGeniusTrie::BranchPtr &parent,
        uint8_t                           idx ) const
    {
    ...
    outcome::result<SuperGeniusTrie::NodePtr> TrieSerializerImpl::retrieveNode( const base::Buffer &db_key ) const
    {
