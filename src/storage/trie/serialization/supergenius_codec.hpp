...
namespace sgns::storage::trie
{
    ...
    public:
        ...
        outcome::result<std::shared_ptr<Node>> decodeNode( const base::Buffer &encoded_data ) const override;
    ...
    private:
        ...
        outcome::result<std::pair<SuperGeniusNode::Type, size_t>> decodeHeader( BufferStream &stream ) const;

        outcome::result<KeyNibbles> decodePartialKey( size_t nibbles_num, BufferStream &stream ) const;

        outcome::result<std::shared_ptr<Node>> decodeBranch( SuperGeniusNode::Type type,
