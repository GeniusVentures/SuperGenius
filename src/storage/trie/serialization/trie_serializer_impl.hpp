...
namespace sgns::storage::trie
{
    ...
    public:
        ...
        outcome::result<std::unique_ptr<SuperGeniusTrie>> retrieveTrie( const base::Buffer &db_key ) const override;
     ...
     */
        ...
        outcome::result<base::Buffer> storeNode( SuperGeniusNode &node, BufferBatch &batch );
     ...
     */
        outcome::result<SuperGeniusTrie::NodePtr> retrieveNode( const base::Buffer &db_key ) const;
        /**
     * Retrieves a node child, replacing a dummy node to an actual node if
     * needed
     */
        outcome::result<SuperGeniusTrie::NodePtr> retrieveChild( const SuperGeniusTrie::BranchPtr &parent,
                                                                 uint8_t                           idx ) const;
