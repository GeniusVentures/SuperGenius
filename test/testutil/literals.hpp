...
inline sgns::common::Buffer operator"" _buf(const char *c, size_t s) {
  std::vector<uint8_t> chars(c, c + s);
  return sgns::common::Buffer(std::move(chars));
}

inline sgns::common::Hash256 operator"" _hash256(const char *c, size_t s) {
  sgns::common::Hash256 hash{};
...
inline sgns::common::Buffer operator"" _hex2buf(const char *c, size_t s) {
  return sgns::common::Buffer::fromHex(std::string_view(c, s)).value();
}
