...
namespace sgns::storage::trie
{
    ...
    namespace printer_internal
    {
        ...
        public:
                               ...
                               size_t                          nest_level = 0 )
            {
                ...
                switch ( node->getTrieType() )
                {
                    ...
                    case T::Leaf:
                    {
                        stream_ << std::setfill( '-' ) << std::setw( nest_level ) << "" << std::setw( 0 )
                                << "(leaf) key: <" << hex_lower( codec_.nibblesToKey( node->key_nibbles ) )
        ...
        private:
            void printBranch( const SuperGeniusTrie::BranchPtr &node, const SuperGeniusTrie &trie, size_t nest_level )
            {
                ...
                stream_ << std::setfill( '-' ) << std::setw( nest_level ) << "" << std::setw( 0 ) << "(branch) key: <"
                        << hex_lower( codec_.nibblesToKey( node->key_nibbles ) ) << "> value: " << value
                        << " children: ";
            ...
            void printEncAndHash( const SuperGeniusTrie::NodePtr &node, size_t nest_level )
            {
                auto enc = codec_.encodeNode( *node ).value();
                if ( print_enc_ )
                {
                    stream_ << std::setfill( '-' ) << std::setw( nest_level ) << "" << std::setw( 0 ) << "enc: " << enc
                            << "\n";
                }
                if ( print_hash_ )
                {
