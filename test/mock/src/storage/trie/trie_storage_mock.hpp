...
namespace sgns::storage::trie
{
    ...
    {
    public:
        MOCK_METHOD0( getPersistentBatch, outcome::result<std::unique_ptr<PersistentTrieBatch>>() );
        MOCK_CONST_METHOD0( getEphemeralBatch, outcome::result<std::unique_ptr<EphemeralTrieBatch>>() );

        MOCK_METHOD1( getPersistentBatchAt,
                      outcome::result<std::unique_ptr<PersistentTrieBatch>>( const base::Hash256 &root ) );
        MOCK_CONST_METHOD1( getEphemeralBatchAt,
                            outcome::result<std::unique_ptr<EphemeralTrieBatch>>( const base::Hash256 &root ) );
