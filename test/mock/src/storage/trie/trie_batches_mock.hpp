...
namespace sgns::storage::trie
{
    ...
    {
    public:
        MOCK_CONST_METHOD1( get, outcome::result<base::Buffer>( const base::Buffer & ) );

        MOCK_METHOD0( cursor, std::unique_ptr<BufferMapCursor>() );

        MOCK_CONST_METHOD1( contains, bool( const base::Buffer & ) );

        MOCK_METHOD2( put, outcome::result<void>( const base::Buffer &, const base::Buffer & ) );
        ...
        MOCK_METHOD2( put_rvalueHack, outcome::result<void>( const base::Buffer &, base::Buffer ) );
