...
namespace sgns::authority
{
    struct AuthorityManagerMock : public AuthorityManager
    {
        MOCK_METHOD1(
            authorities,
            outcome::result<std::shared_ptr<const primitives::AuthorityList>>( const primitives::BlockInfo & ) );
        ...
        MOCK_METHOD2( applyOnDisabled, outcome::result<void>( const primitives::BlockInfo &, uint64_t ) );

        MOCK_METHOD2( applyPause, outcome::result<void>( const primitives::BlockInfo &, primitives::BlockNumber ) );

        MOCK_METHOD2( applyResume, outcome::result<void>( const primitives::BlockInfo &, primitives::BlockNumber ) );
