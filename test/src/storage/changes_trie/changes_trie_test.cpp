...
TEST( ChangesTrieTest, SubstrateCompatibility )
{
    ...
    std::map<Buffer, std::vector<sgns::primitives::ExtrinsicIndex>> changes{ { Buffer{ 1 }, { 1 } },
                                                                             { ":extrinsic_index"_buf, { 1 } } };

    auto changes_trie = ChangesTrie::buildFromChanges( 99, factory, codec, changes, ChangesTrieConfig{} ).value();
    auto hash         = changes_trie->getHash();
    ASSERT_EQ(
        hash,
        sgns::base::Hash256::fromHex( "bb0c2ef6e1d36d5490f9766cfcc7dfe2a6ca804504c3bb206053890d6dd02376" ).value() );
}
