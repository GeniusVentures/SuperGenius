...
class TrieTest : public testing::Test, public ::testing::WithParamInterface<std::vector<TrieCommand>>
{
public:
    TrieTest() {}
    ...
    void SetUp() override
    {
        trie = std::make_unique<SuperGeniusTrieImpl>();
    }
...
const std::vector<std::pair<Buffer, Buffer>> TrieTest::data = { { "123456"_hex2buf, "42"_hex2buf },
                                                                { "1234"_hex2buf, "1234"_hex2buf },
                                                                { "010203"_hex2buf, "0a0b"_hex2buf },
                                                                { "010a0b"_hex2buf, "1337"_hex2buf },
                                                                { "0a0b0c"_hex2buf, "deadbeef"_hex2buf } };

void FillSmallTree( SuperGeniusTrie &trie )
{
        ...
        switch ( command.command )
        {
            ...
            case Command::GET:
            {
                ...
                else
                {
                    ...
                    ASSERT_EQ( err.value(), static_cast<int>( sgns::storage::trie::TrieError::NO_VALUE ) );
                }
                break;
            }
            case Command::PUT:
 ...
 */
std::vector<TrieCommand> BuildSmallTrie = { { "0135"_hex2buf, "pen"_buf, Command::PUT },
 ...
 * accessible
 */
std::vector<TrieCommand> PutAndGetBranch = { { "0135"_hex2buf, ( "spaghetti"_buf ), Command::PUT },
 ...
 */
std::vector<TrieCommand> PutAndGetOddKeyLengths = { { "43c1"_hex2buf, "noot"_buf, Command::PUT },
 ...
 */
std::vector<TrieCommand> DeleteFromBranch = { { "0615fc"_hex2buf, "noot"_buf, Command::PUT },
 ...
 */
std::vector<TrieCommand> DeleteOddKeyLengths = { { "43c1"_hex2buf, "noot"_buf, Command::PUT },
                                                 { "43c1"_hex2buf, "noot"_buf, Command::GET },
                                                 { "4929"_hex2buf, "nootagain"_buf, Command::PUT },
                                                 { "4929"_hex2buf, "nootagain"_buf, Command::GET },
                                                 { "430c"_hex2buf, "odd"_buf, Command::PUT },
...
TEST_F( TrieTest, GetPath )
{
    ...
    const std::vector<std::pair<Buffer, Buffer>> data = { { "123456"_hex2buf, "42"_hex2buf },
                                                          { "1234"_hex2buf, "1234"_hex2buf },
                                                          { "010203"_hex2buf, "0a0b"_hex2buf },
                                                          { "010a0b"_hex2buf, "1337"_hex2buf },
                                                          { "0a0b0c"_hex2buf, "deadbeef"_hex2buf } };
...
TEST_F( TrieTest, GetPathToInvalid )
{
    ...
    EXPECT_OUTCOME_FALSE( path, trie->getPath( trie->getRoot(), KeyNibbles{ "0a0b0c0d0e0f"_hex2buf } ) );
}
