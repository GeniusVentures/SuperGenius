...
using sgns::storage::trie::operator<<;

class SuperGeniusTrieCursorTest : public testing::Test
{
};
    ...
    size_t key_alphabet_size = 16 ) noexcept
{
    ...
    std::uniform_int_distribution<std::mt19937::result_type>       key_dist( 0, key_alphabet_size );
    std::uniform_int_distribution<std::mt19937::result_type>       length_dist( 1, max_key_length );
...
std::shared_ptr<SuperGeniusTrie> makeTrie( const std::vector<std::pair<Buffer, Buffer>> &vals )
{
    auto trie = std::make_shared<SuperGeniusTrieImpl>( []( auto b, uint8_t idx ) { return b->children.at( idx ); } );
    for ( auto &p : vals )
    {
...
TEST_F( SuperGeniusTrieCursorTest, NextOnRootOnlyTrie )
{
    auto                  trie = makeTrie( { { "a"_buf, Buffer{ 1 } } } );
    SuperGeniusTrieCursor cursor{ *trie };
...
TEST_F( SuperGeniusTrieCursorTest, NextOnEmptyTrie )
{
    auto                  trie = makeTrie( {} );
    SuperGeniusTrieCursor cursor{ *trie };
    ASSERT_FALSE( cursor.isValid() );
...
TEST_F( SuperGeniusTrieCursorTest, NextOnSmallTrie )
{
...
TEST_F( SuperGeniusTrieCursorTest, BigPseudoRandomTrieRandomStart )
{
    ...
    size_t keys_behind = 0; // number of keys lexicographically lesser than current
