...
struct NodeDecodingTest : public ::testing::TestWithParam<std::shared_ptr<SuperGeniusNode>>
{
...
template <typename T>
std::shared_ptr<SuperGeniusNode> make( const base::Buffer &key_nibbles, const base::Buffer &value )
{
...
{
    auto node         = std::make_shared<BranchNode>( KeyNibbles{ "010203"_hex2buf }, "0a"_hex2buf );
    auto child1       = std::make_shared<LeafNode>( KeyNibbles{ "01"_hex2buf }, "0b"_hex2buf );
    auto child2       = std::make_shared<LeafNode>( KeyNibbles{ "02"_hex2buf }, "0c"_hex2buf );
