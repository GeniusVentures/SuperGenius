...
const std::vector<std::pair<Buffer, Buffer>> TrieBatchTest::data = { { "123456"_hex2buf, "42"_hex2buf },
...
class MockDb : public sgns::storage::InMemoryStorage
{
public:
...
TEST_F( TrieBatchTest, ConsistentOnFailure )
{
   ...
   */
    auto &&expectation =
        EXPECT_CALL( *db, put( _, _ ) ).Times( 5 ).WillRepeatedly( Invoke( db.get(), &MockDb::true_put ) );
    ...
    auto trie  = TrieStorageImpl::createEmpty( factory, codec, serializer, boost::none ).value();
    auto batch = trie->getPersistentBatch().value();
