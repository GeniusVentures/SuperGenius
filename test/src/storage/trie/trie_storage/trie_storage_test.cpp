...
namespace sgns::storage
{
   ...
   */
    TEST( TriePersistencyTest, CreateDestroyCreate )
    {
        ...
        {
            EXPECT_OUTCOME_TRUE( rocks_db, rocksdb::create( "supergenius_rocksdb_persistency_test", options ) );
            auto serializer = std::make_shared<TrieSerializerImpl>(
                factory,
                codec,
                std::make_shared<TrieStorageBackendImpl>( std::move( rocks_db ), kNodePrefix ) );
            auto storage = TrieStorageImpl::createEmpty( factory, codec, serializer, boost::none ).value();
        ...
        EXPECT_OUTCOME_TRUE( new_rocks_db, rocksdb::create( "supergenius_rocksdb_persistency_test", options ) );
        auto serializer = std::make_shared<TrieSerializerImpl>(
            factory,
            codec,
            std::make_shared<TrieStorageBackendImpl>( std::move( new_rocks_db ), kNodePrefix ) );
        auto storage = TrieStorageImpl::createFromStorage( root, codec, serializer, boost::none ).value();
        auto batch   = storage->getPersistentBatch().value();
