...
namespace sgns
{
    ...
    protected:
            ...
            const std::string                   &previous_hash = "" )
        {
            ...
            utxo_manager.ReserveUTXOs( params.first, transfer_transaction->GetHash() );
...
TEST_F( TransactionSyncTest, InvalidPreviousHashTest )
{
    ...
    auto mint_result =
        node_proc1->MintTokens( 20000000000, sgns::test::NextMintSourceHash(), "", TokenID::FromBytes( { 0x00 } ) );
    ASSERT_TRUE( mint_result.has_value() ) << "Mint transaction failed or timed out";
