...
TEST( ProverTest, CreateAssignment )
{
    ...
    auto print_result = GeniusAssigner.PrintCircuitAndTable( assign_result.value(), "./table.tbl", "./circuit.crct" );
...
TEST( ProverTest, CreateAndVerifyProof )
{
    ...
    EXPECT_TRUE( GeniusProver.WriteProofToFile( proof_result.value().proof, "./sgnus_proof.bin" ) );
}
