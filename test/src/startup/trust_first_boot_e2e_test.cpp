...
namespace
{
    ...
    /// Build a GeniusSigner from a 64-hex private key (merged GeniusSigner is
    /// libsecp256k1-based and takes the raw 32-byte big-endian secret key).
    sgns::GeniusSigner SignerFromHex( std::string_view hex_key )
    {
        sgns::GeniusSigner::PrivateKey private_key{};
        auto                          bytes = sgns::base::unhex( hex_key );
        if ( !bytes.has_error() && bytes.value().size() == private_key.size() )
        {
            std::copy_n( bytes.value().begin(), private_key.size(), private_key.begin() );
        }
        return sgns::GeniusSigner( private_key );
    }
    ...
    TEST( TrustFirstBootE2ETest, ProductionControllerInitiatesBurnAndRestartRecoversPersistedApprovals )
    {
        ...
        sgns::GeniusSigner bootstrapper{ SignerFromHex( BOOTSTRAPPER_PRIVATE_KEY ) };
