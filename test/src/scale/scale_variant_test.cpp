...
class VariantFixture : public testing::TestWithParam<std::pair<boost::variant<uint8_t, uint32_t>, ByteArray>>
{
protected:
...
namespace
{
    std::pair<boost::variant<uint8_t, uint32_t>, ByteArray> make_pair( boost::variant<uint8_t, uint32_t> v,
                                                                       ByteArray                         m )
    {
        return std::pair<boost::variant<uint8_t, uint32_t>, ByteArray>( std::move( v ), std::move( m ) );
    }
} // namespace
...
INSTANTIATE_TEST_SUITE_P( CompactTestCases,
                          ...
                          ::testing::Values( make_pair( uint8_t( 1 ), { 0, 1 } ),
                                             make_pair( uint32_t( 2 ), { 1, 2, 0, 0, 0 } ) ) );
...
TEST( ScaleVariant, DecodeU8Success )
{
    ...
    sgns::visit_in_place( val, []( uint8_t v ) { ASSERT_EQ( v, 1 ); }, []( uint32_t v ) { FAIL(); } );
}
...
TEST( ScaleVariant, DecodeU32Success )
{
    ...
    boost::variant<uint8_t, uint32_t> val{};
    ASSERT_NO_THROW( s >> val );
    sgns::visit_in_place( val, []( uint32_t v ) { ASSERT_EQ( v, 1 ); }, []( uint8_t v ) { FAIL(); } );
}
