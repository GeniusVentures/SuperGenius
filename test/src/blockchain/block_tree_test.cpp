...
#include "mock/src/api/service/author/author_api_mock.hpp"
#include "mock/src/blockchain/block_header_repository_mock.hpp"
#include "mock/src/blockchain/block_storage_mock.hpp"
...
public:
    void SetUp() override
    {
        ...
        EXPECT_CALL( *storage_, getBlockHeader( kLastFinalizedBlockId ) ).WillOnce( Return( finalized_block_header_ ) );

        block_tree_ =
            BlockTreeImpl::create( header_repo, storage_, kLastFinalizedBlockId, extrinsic_observer_, hasher ).value();
    }
   ...
   */
    BlockHash addBlock( const Block &block )
    {
        ...
        auto hash          = hasher->blake2b_256( encoded_block );

        EXPECT_CALL( *storage_, putBlock( block ) ).WillRepeatedly( Return( hash ) );
        EXPECT_TRUE( block_tree_->addBlock( block ) );

        EXPECT_CALL( *header_repo, getBlockHeader( primitives::BlockId( hash ) ) )
            .WillRepeatedly( Return( block.header ) );
        EXPECT_CALL( *header_repo, getHashByNumber( block.header.number ) ).WillRepeatedly( Return( hash ) );
    ...
    BlockHash finalizedBlockHash = BlockHash::fromString( "andj4kdn4odnfkslfn3k4jdnbmeodkv4" ).value();

    BlockInfo finalizedBlockInfo{ 42U, finalizedBlockHash };

    std::shared_ptr<BlockHeaderRepositoryMock> header_repo = std::make_shared<BlockHeaderRepositoryMock>();

    std::shared_ptr<BlockStorageMock> storage_ = std::make_shared<BlockStorageMock>();

    std::shared_ptr<network::ExtrinsicObserver> extrinsic_observer_ =
        std::make_shared<network::ExtrinsicObserverImpl>( std::make_shared<api::AuthorApiMock>() );

    std::shared_ptr<crypto::Hasher> hasher = std::make_shared<crypto::HasherImpl>();

    std::shared_ptr<BlockTreeImpl> block_tree_;

    BlockId kLastFinalizedBlockId = finalizedBlockHash;
...
TEST_F( BlockTreeTest, AddBlock )
{
    ...
    ASSERT_EQ( deepest_block_hash, finalizedBlockHash );

    auto leaves = block_tree_->getLeaves();
    ASSERT_EQ( leaves.size(), 1 );
    ASSERT_EQ( leaves[0], finalizedBlockHash );

    auto children_res = block_tree_->getChildren( finalizedBlockHash );
    ASSERT_TRUE( children_res );
    ASSERT_TRUE( children_res.value().empty() );

    // WHEN
    BlockHeader header{ finalizedBlockHash, 1, {}, {}, { PreRuntime{} } };
...
TEST_F( BlockTreeTest, Finalize )
{
    ...
    ASSERT_EQ( last_finalized_hash, finalizedBlockHash );

    BlockHeader header{ /*.parent_hash =*/finalizedBlockHash,
                        /*.number =*/1,
                        {},
                        {},
                        /*.digest =*/{ PreRuntime{} } };
    ...
    EXPECT_CALL( *storage_, setLastFinalizedBlockHash( hash ) ).WillRepeatedly( Return( outcome::success() ) );
...
TEST_F( BlockTreeTest, GetChainByBlockOnly )
{
    ...
    header     = BlockHeader{ /*.parent_hash =*/hash1, /*.number =*/2, {}, {}, /*.digest =*/{ Verification{} } };
    body       = BlockBody{ { Buffer{ 0x55, 0x55 } } };
    new_block  = Block{ header, body };
    auto hash2 = addBlock( new_block );

    std::vector<BlockHash> expected_chain{ finalizedBlockHash, hash1, hash2 };
...
TEST_F( BlockTreeTest, GetChainByBlockAscending )
{
    ...
    std::vector<BlockHash> expected_chain{ hash2, hash1, finalizedBlockHash };

    EXPECT_CALL( *header_repo, getNumberByHash( hash2 ) ).WillOnce( Return( 2 ) );
    EXPECT_CALL( *header_repo, getHashByNumber( 0 ) ).WillOnce( Return( finalizedBlockHash ) );
...
TEST_F( BlockTreeTest, GetChainByBlockDescending )
{
    ...
    EXPECT_CALL( *header_repo, getNumberByHash( finalizedBlockHash ) ).WillOnce( Return( 0 ) );
    EXPECT_CALL( *header_repo, getHashByNumber( 2 ) ).WillOnce( Return( hash2 ) );

    // WHEN
    EXPECT_OUTCOME_TRUE( chain, block_tree_->getChainByBlock( finalizedBlockHash, false, 5 ) );
...
{
    EXPECT_CALL( *header_repo, getBlockHeader( kLastFinalizedBlockId ) )
    ...
    EXPECT_CALL( *header_repo, getBlockHeader( primitives::BlockId( target_hash ) ) )
        .WillRepeatedly( Return( target_header ) );
    EXPECT_CALL( *header_repo, getHashByNumber( target_header.number ) ).WillRepeatedly( Return( target_hash ) );

    EXPECT_OUTCOME_FALSE( best_info, block_tree_->getBestContaining( target_hash, boost::none ) );
...
TEST_F( BlockTreeTest, GetBestChain_ShortChain )
{
    ...
    EXPECT_OUTCOME_TRUE( best_info, block_tree_->getBestContaining( target_hash, boost::none ) );
