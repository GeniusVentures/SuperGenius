...
inline sgns::TransportFactory GetTransportFactory( const std::vector<MockEndpointConfig> &endpoint_configs )
{
    ...
    {
        return []( const std::string &url, std::chrono::seconds timeout ) -> std::unique_ptr<eth::rpc::JsonRpcTransport> {
    ...
    return [endpoint_configs]( const std::string &url, std::chrono::seconds /*timeout*/ ) -> std::unique_ptr<eth::rpc::JsonRpcTransport> {
