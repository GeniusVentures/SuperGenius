...
def main():
    parser = argparse.ArgumentParser(description="Create a tiny mat4 ONNX model and reference output.")
    parser.add_argument("--matrices", type=int, default=10, help="Number of mat4 entries")
    parser.add_argument("--patch", type=int, default=5, help="Patch length (block_len)")
    parser.add_argument("--stride", type=int, default=2, help="Stride in matrices")
    parser.add_argument("--out-dir", default=".", help="Output directory")
    parser.add_argument("--onnx", default="mat4_tiny.onnx", help="ONNX filename")
    parser.add_argument("--raw", default="mat4_input.raw", help="Raw input filename")
    parser.add_argument("--pt", default="mat4_tiny_state.pt", help="Torch state dict filename")
    parser.add_argument("--ref", default="mat4_output_pt.raw", help="Reference output filename")
    ...
    model = TinyMat4Net().eval()

    input_mats = rng.normal(size=(args.matrices, 16)).astype(np.float32)
    raw_path = os.path.join(args.out_dir, args.raw)
    input_mats.reshape(-1).tofile(raw_path)

    dummy = torch.randn(1, 16, args.patch, dtype=torch.float32)
    ...
    output_accum = np.zeros((16, args.matrices), dtype=np.float32)
    ...
    with torch.no_grad():
        ...
        for start in starts:
            patch = np.zeros((16, args.patch), dtype=np.float32)
