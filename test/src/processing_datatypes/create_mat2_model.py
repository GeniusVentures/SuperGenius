...
def main():
    parser = argparse.ArgumentParser(description="Create a tiny mat2 ONNX model and reference output.")
    parser.add_argument("--matrices", type=int, default=16, help="Number of mat2 entries")
    parser.add_argument("--patch", type=int, default=8, help="Patch length (block_len)")
    parser.add_argument("--stride", type=int, default=4, help="Stride in matrices")
    parser.add_argument("--out-dir", default=".", help="Output directory")
    parser.add_argument("--onnx", default="mat2_tiny.onnx", help="ONNX filename")
    parser.add_argument("--raw", default="mat2_input.raw", help="Raw input filename")
    parser.add_argument("--pt", default="mat2_tiny_state.pt", help="Torch state dict filename")
    parser.add_argument("--ref", default="mat2_output_pt.raw", help="Reference output filename")
    ...
    model = TinyMat2Net().eval()

    input_mats = rng.normal(size=(args.matrices, 4)).astype(np.float32)
    raw_path = os.path.join(args.out_dir, args.raw)
    input_mats.reshape(-1).tofile(raw_path)

    dummy = torch.randn(1, 4, args.patch, dtype=torch.float32)
    ...
    output_accum = np.zeros((4, args.matrices), dtype=np.float32)
    ...
    with torch.no_grad():
        ...
        for start in starts:
            patch = np.zeros((4, args.patch), dtype=np.float32)
