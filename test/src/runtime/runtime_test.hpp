...
public:
    ...
    void SetUp() override
    {
        ...
        auto storage_provider = std::make_shared<sgns::runtime::TrieStorageProviderMock>();
        ON_CALL( *storage_provider, getCurrentBatch() )
            .WillByDefault( testing::Invoke( []() { return std::make_unique<PersistentTrieBatchMock>(); } ) );
        ON_CALL( *storage_provider, tryGetPersistentBatch() )
            .WillByDefault( testing::Invoke(
                []() -> boost::optional<std::shared_ptr<PersistentTrieBatch>>
                { return std::shared_ptr<PersistentTrieBatch>( std::make_shared<PersistentTrieBatchMock>() ); } ) );
        ON_CALL( *storage_provider, setToPersistent() ).WillByDefault( testing::Return( outcome::success() ) );
        ON_CALL( *storage_provider, setToEphemeral() ).WillByDefault( testing::Return( outcome::success() ) );

        auto random_generator   = std::make_shared<sgns::crypto::BoostRandomGenerator>();
        auto sr25519_provider   = std::make_shared<sgns::crypto::SR25519ProviderImpl>( random_generator );
        auto ed25519_provider   = std::make_shared<sgns::crypto::ED25519ProviderImpl>();
        auto secp256k1_provider = std::make_shared<sgns::crypto::Secp256k1ProviderImpl>();
        auto hasher             = std::make_shared<sgns::crypto::HasherImpl>();
        auto pbkdf2_provider    = std::make_shared<sgns::crypto::Pbkdf2ProviderImpl>();
        auto bip39_provider     = std::make_shared<sgns::crypto::Bip39ProviderImpl>( pbkdf2_provider );
        auto crypto_store       = std::make_shared<sgns::crypto::CryptoStoreImpl>( ed25519_provider,
                                                                             sr25519_provider,
                                                                             secp256k1_provider,
                                                                             bip39_provider,
                                                                             random_generator );
        changes_tracker_        = std::make_shared<sgns::storage::changes_trie::ChangesTrackerMock>();

        auto extension_factory = std::make_shared<sgns::extensions::ExtensionFactoryImpl>(
            changes_tracker_,
            sr25519_provider,
            ed25519_provider,
            secp256k1_provider,
            ...
            crypto_store,
            bip39_provider,
            [this]( std::shared_ptr<sgns::runtime::WasmProvider> wasm_provider )
            {
                sgns::runtime::binaryen::CoreFactoryImpl factory(
                    runtime_manager_,
                    changes_tracker_,
                    std::make_shared<sgns::blockchain::BlockHeaderRepositoryMock>() );
                return factory.createWithCode( std::move( wasm_provider ) );
            } );

        auto module_factory = std::make_shared<sgns::runtime::binaryen::WasmModuleFactoryImpl>();

        auto wasm_path = boost::filesystem::path( __FILE__ ).parent_path().string() + "/wasm/sgns_runtime.compact.wasm";
        wasm_provider_ = std::make_shared<sgns::runtime::BasicWasmProvider>( wasm_path );

        runtime_manager_ = std::make_shared<sgns::runtime::binaryen::RuntimeManager>( std::move( extension_factory ),
    ...
    sgns::primitives::BlockHeader createBlockHeader()
    {
        ...
        return BlockHeader{ parent_hash, number, state_root, extrinsics_root, digest };
    }
...
protected:
    ...
    std::shared_ptr<sgns::storage::changes_trie::ChangesTracker> changes_tracker_;
};
