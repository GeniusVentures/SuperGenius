...
protected:
    ...
    /// @param rpcCatchup Persisted into sgns_config.json's "rpc_catchup" (phase-13 catch-up gating).
                                                  ...
                                                  bool                       rpcCatchup          = false )
    {
        ...
        // Merged GeniusAccount::NewFromPrivateKey takes no full-node flag: the role
        // comes from the node_type written into sgns_config.json below.
        ...
        if ( !reuseStorage )
        {
            ...
            // node_type honors the Archive override (ours) inside the phase-13 trust config.
...
{
    auto node = CreateNode( "concurrent_account_generation",
                            true,
                            false,
                            true,
                            {},
                            /*nodeTypeOverride=*/std::nullopt,
                            /*rpcCatchup=*/true );
    ASSERT_TRUE( node );
    WaitForReady( node );
