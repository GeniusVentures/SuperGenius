...
namespace
{
    ...
    protected:
        void SetUp() override
        {
            ...
            for ( size_t i = 0; i < 3; ++i ) signers_.push_back( GeniusSigner::Generate() );
        ...
        void Construct( size_t local_signer = 0 )
        {
            ...
            auto created = TrustedPeerRegistry::NewProduction(
                secure_crdt_, store_, manifest, signers_[0].Sign( bytes ), signers_[local_signer].GetAddress(),
...
TEST_F( OperatorApprovalTest, FreshRegistryIsEmptyAndPolicyOperationsRequireConfirmedGenesis )
{
    ...
    EXPECT_TRUE( registry_->ProposePolicyCandidate( Manifest().Canonicalized().has_value()
                                                        ? QuorumPolicyState{}
                                                        : QuorumPolicyState{} ).has_error() );
}
...
TEST_F( OperatorApprovalTest, ReceiptNeverSignsAndExplicitApprovalIsDeduplicated )
{
    ...
    securecrdt::CandidateApprovalRecord remote{
...
TEST_F( OperatorApprovalTest, ProposedPeersCannotSelfAuthorizeAndWrongLinksNeverReachStore )
{
    ...
    candidate.peers = { signers_[2].GetAddress() };
    candidate.membership_threshold = 1;
    candidate.burn_threshold = 1;
    const auto core = TrustedPeerRegistry::PolicyCandidateCore( candidate ).value();
    const auto bytes = core.CanonicalBytes().value();
    securecrdt::CandidateApprovalRecord self_authorized{
        securecrdt::CandidateApprovalRecord::ENCODING_VERSION,
        core,
        signers_[2].GetAddress(),
        signers_[2].Sign( bytes )
    };
